...
public class ElkService implements InitializingBean {
    ...
    public void saveIdxStatusWriteDB(String indexName, String type, Long objId) throws AxmrGenericException {
        ...
        c.add(Restrictions.eq("indexName", indexName)).add(Restrictions.eq("objType", type)).add(Restrictions.eq("objId", objId));
        ...
        Log.debug(this.getClass(),"SAVEIDXSTATUS-DONE: "+indexName+" - "+type+" - "+objId);
    }
    public void saveIdxStatus(String indexName, String type, Long objId) throws AxmrGenericException {
        ...
        //}else{
            //VMAZZEO//Log.info(this.getClass()," - effettuo scrittura diretta su DB");
    ...
    public void simpleIndex(Element el) throws UnknownHostException, AxmrGenericException {
        ...
        Log.debug(this.getClass()," - Controllo che l'indice dell'elemento non sia stato aggiornato nel frattempo el:"+el.getId()+" di tipo "+el.getTypeName());
        isDao = new BaseDao<ElkIndexesStatus>(docService.getTxManager(), "doc", ElkIndexesStatus.class);

        Criteria c=isDao.getCriteria();
        c.add(Restrictions.eq("indexName", "simple")).add(Restrictions.eq("objType", el.getTypeName())).add(Restrictions.eq("objId", el.getId()));
        ElkIndexesStatus elIdx= (ElkIndexesStatus) c.uniqueResult();
        if (elIdx!=null){
            if (el.getLastUpdateDt()!=null){
                if (elIdx.getLastUpdateDt().after(el.getLastUpdateDt())) {
                    Log.debug(this.getClass()," -  - - Escludo in quanto data di incizzazione e successiva a data di ultima modifica el:"+el.getId()+" di tipo "+el.getTypeName());
                    return;
                }
            }else {
                if (elIdx.getLastUpdateDt().after(el.getCreationDt())) {
                    Log.debug(this.getClass()," -  - - Escludo in quanto data di incizzazione e successiva a data di creazione elemento non modificato el:"+el.getId()+" di tipo "+el.getTypeName());
                    return;
                }
            }
        }
        ...
        if (el.isDeleted()){
            ...
            if (response != null && response.getShardInfo().getFailed()==0){ // && (response.isCreated() || !response.getIndex().isEmpty() || !response.getId().isEmpty())) {
                ...
                //Index error
				Log.error(this.getClass(),"Index error");
				//Log.debug(this.getClass(),"RESPONSE: "+response);
            }
    ...
    public void elementIdxUpdate(Element el) throws AxmrGenericException, UnknownHostException {
        //VMAZZEO//Log.debug(this.getClass()," - - ### Avvio indicizzazione elementIdxUpdate elemento "+el.getId()+" ("+el.getTypeName()+")");
    ...
    public void fieldIndex(Element el) throws UnknownHostException, AxmrGenericException {
        if (fieldIndexObjsList.size()>0 && !fieldIndexObjsList.contains(el.getTypeName().toLowerCase())){
            //VMAZZEO//Log.debug(this.getClass()," - ###ELK escludo indicizzazione fields elemento "+el.getId()+" di tipo "+el.getTypeName());
            return;
        }else if (fieldIndexObjsList.size()==0 && !el.getType().isSearchable()){
            //VMAZZEO//Log.debug(this.getClass()," - ###ELK escludo indicizzazione (isSearchable) fields elemento "+el.getId()+" di tipo "+el.getTypeName());
            return;
        }
        //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX! - elemento "+el.getId()+" di tipo "+el.getTypeName());
        String type=el.getTypeName();
        Log.debug(this.getClass()," - Controllo che l'indice dell'elemento non sia stato aggiornato nel frattempo el:"+el.getId()+" di tipo "+el.getTypeName());
        isDao = new BaseDao<ElkIndexesStatus>(docService.getTxManager(), "doc", ElkIndexesStatus.class);
        Criteria c=isDao.getCriteria();
        c.add(Restrictions.eq("indexName", "field")).add(Restrictions.eq("objType", el.getTypeName())).add(Restrictions.eq("objId", el.getId()));
        ElkIndexesStatus elIdx= (ElkIndexesStatus) c.uniqueResult();
        if (elIdx!=null){
            if (el.getLastUpdateDt()!=null){
                if (elIdx.getLastUpdateDt().after(el.getLastUpdateDt())) {
        ...
        //VMAZZEO//Log.debug(this.getClass(),"PREPARE indice fields - tipo "+type.toLowerCase()+" - id: "+el.getId());
        ...
        //VMAZZEO//Log.debug(this.getClass(), new Gson().toJson(maps));
        ...
        if (el.isDeleted()){
            //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX 1 - elemento "+el.getId()+" eliminato !!!");
            ...
            if (response.getHits().getTotalHits()>0){
                for (int i=0;i<response.getHits().getHits().length;i++){
                    ...
                    Log.debug(this.getClass(),"PROVO AD ELIMINARE: "+indexPrefix.toLowerCase() + "-fields "+type.toLowerCase()+" "+id);
                    ...
                    if (delResponse != null && delResponse.getShardInfo().getFailed()==0){ // && delResponse.isFound()){
                        //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX 1.1 - trovo elemento "+el.getId()+" e lo elimino !!!");
                ...
                //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX 1.2 - non trovo elemento "+el.getId()+" ma aggiorno indice per non riprocessarlo nuovamente!!!");
            //    saveIdxStatus("fields", type, el.getId());
            }
            saveIdxStatus("fields", type, el.getId());
        }else {
            //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX 2 recupero valori - elemento "+el.getId()+" di tipo "+el.getTypeName());
            List<ElkValue> vals = ElkValue.buildValues(el);
            //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX 3 trovati "+vals.size()+" valori - elemento "+el.getId()+" di tipo "+el.getTypeName());
            boolean error=false;
            ...
            //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX 4 controllo condizioni per salvare indice - elemento "+el.getId()+" di tipo "+el.getTypeName());
            if (!error) {
                //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX 5 salvo indice in DB- elemento "+el.getId()+" di tipo "+el.getTypeName());
                saveIdxStatus("fields", type, el.getId());
            }else {
                //VMAZZEO//Log.debug(this.getClass(),"#### FIELD-INDEX 6 mancano condizioni per salvare indice ("+vals.size()+")- elemento "+el.getId()+" di tipo "+el.getTypeName());
    ...
    public void fullIndex(Element el) throws UnknownHostException, AxmrGenericException {
        //Log.debug(this.getClass()," - FULL-INDEX! - elemento "+el.getId()+" di tipo "+el.getTypeName()+" controllo se indicizzabile...");
        if (fullIndexObjsList.size()>0 && !fullIndexObjsList.contains(el.getTypeName().toLowerCase())){
            //Log.debug(this.getClass()," - escludo indicizzazione full elemento "+el.getId()+" di tipo "+el.getTypeName());
            return;
        }else if (fullIndexObjsList.size()==0 && !el.getType().isSearchable()){
            //Log.debug(this.getClass()," - escludo indicizzazione (isSearchable) full elemento "+el.getId()+" di tipo "+el.getTypeName());
            return;
        }
		//VMAZZEO//Log.debug(this.getClass()," - - - elemento "+el.getId()+" di tipo "+el.getTypeName()+" indicizzo...");
        String type = el.getTypeName();
        Log.debug(this.getClass()," - Controllo che l'indice dell'elemento non sia stato aggiornato nel frattempo el:"+el.getId()+" di tipo "+el.getTypeName());
        isDao = new BaseDao<ElkIndexesStatus>(docService.getTxManager(), "doc", ElkIndexesStatus.class);

        Criteria c=isDao.getCriteria();
        c.add(Restrictions.eq("indexName", "full")).add(Restrictions.eq("objType", type)).add(Restrictions.eq("objId", el.getId()));
        ElkIndexesStatus elIdx= (ElkIndexesStatus) c.uniqueResult();
        if (elIdx!=null){
            if (el.getLastUpdateDt()!=null){
                if (elIdx.getLastUpdateDt().after(el.getLastUpdateDt())) {
        ...
        //Log.debug(this.getClass(),"#### FULL-INDEX! - elemento "+el.getId()+" di tipo "+el.getTypeName());
        Client elkClient = getNewClient();
        ElkSimpleElement tsel = new ElkSimpleElement();
        ...
        //Log.debug(this.getClass(),"PREPARE indice simple - tipo "+type.toLowerCase()+" - id: "+el.getId());
        Map<String, Object> maps = new HashMap<String, Object>();
        maps.put("date_detection", true);
        LinkedList<Object> templates = new LinkedList<Object>();
        Map<String,ElkTemplate> singleTemplate = new HashMap<String,ElkTemplate>();
        ...
        templates.add(singleTemplate);
        maps.put("dynamic_templates",templates);
        //Log.debug(this.getClass(), new Gson().toJson(maps));
        ...
        //Log.debug(this.getClass(),"PREPARE indice full - tipo "+type.toLowerCase()+" - id: "+el.getId());
        ...
        if (el.isDeleted()){
            ...
            /*
            Log.debug(this.getClass(),"PREPARE indice simple - tipo "+type.toLowerCase()+" - id: "+el.getId());
            Log.debug(this.getClass(),new Gson().toJson(sel.getMapping()));
            ...
            */
			...
			//Log.debug(this.getClass(),"PUT indice simple - tipo "+type.toLowerCase()+" - id: "+el.getId());
            IndexRequest indexRequest = new IndexRequest(indexPrefix.toLowerCase() + "-simple", type.toLowerCase(), el.getId() + "");
            //IndexRequest indexRequest = new IndexRequest(indexPrefix.toLowerCase() + "-simple", type.toLowerCase()).prep;
            //Log.debug(this.getClass(),new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create().toJson(sel));
            ...
            if (fullIndexObjsList.size()>0){
                    //Log.debug(this.getClass()," - - Elemento "+el.getTypeName()+" lista elementi presente");
                    cel = ElkFullElement.simpleToComplex(sel, el, true, fullIndexObjsList, fullIndexWithParentsObjsList);
            }else {
                //Log.debug(this.getClass()," - - Elemento "+el.getTypeName()+" lista elementi NON presente");
                cel = ElkFullElement.simpleToComplex(sel, el, true, null, null);
            }
            /*
            Log.debug(this.getClass(),"PREPARE indice full - tipo "+type.toLowerCase()+" - id: "+el.getId());
            Log.debug(this.getClass(),new Gson().toJson(cel.getMapping()));
            putMappingResponse = getClient().admin().indices()
                    .preparePutMapping(indexPrefix.toLowerCase() + "-full")
                    .setType(type.toLowerCase())
                    ...
                    .execute().actionGet();
            */
			//Log.debug(this.getClass(),"PUT indice full - tipo "+type.toLowerCase()+" - id: "+el.getId());
    ...
    public void recursiveFullIndex(ElkFullElement elk) throws AxmrGenericException, UnknownHostException {
		...
		//Log.debug(this.getClass(),"PUT indice full - tipo "+type.toLowerCase()+" - id: "+elk.getId());
        ...
        IndexResponse response = elkClient.index(indexRequest).actionGet();
        //Log.debug(this.getClass(),"RESPONSE: "+response);
        Log.debug(this.getClass(),"Processo figli da indicizzare");
        ...
        //Log.debug(this.getClass(),"PUT indice simple - tipo "+type.toLowerCase()+" - id: "+elk.getId());
    ...
    public void doFullIndex(String type, boolean forceAll) throws UnknownHostException, AxmrGenericException {
        Log.debug(this.getClass(),"####DO FULL INDEX!!");
        Long typeId = docService.getTypeIdByNameOrId(type);
        Criteria c=docService.getDocDAO().getCriteria("main");
        c.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY)
                .add(Restrictions.eq("type.id", typeId));
        ...
        if (els!=null) {
            for (Element el : els) {
                //Log.debug(this.getClass()," - - ElkFullIndex - indicizzo elemento di tipo "+el.getType().getTypeId()+" - obj_id:"+el.getId());
                fullIndex(el);
            }
        }
    }
    ...
    public SearchResponse doElkQuery(IUser user, String indexType, String type, String filter,String sidx, String sord, int page, int rpp, boolean fields) throws UnknownHostException {
        ...
        //VMAZZEO//Log.debug(this.getClass(),requestBuilder.toString());
        SearchResponse response=requestBuilder.execute().actionGet();
    ...
    public List<FieldSearchResult> fullsearch(IUser user, String pattern) throws RestException {
        try {
            ...
            //Log.debug(getClass(), "Search " + indexPrefix + " ...");
            //Log.debug(getClass(), "Search " + query + " ...");
