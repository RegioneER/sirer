...
function changeTipologia(){
	...
	if(tipologia==1 || true){
		...
		$( "#Prestazioni_CDC2" ).click(function(){
    ...
    $.ajax({
        ...
        success: function(obj){
            if (obj.result=="OK") {
                ...
                loadingScreen("Clonazione effettuata", "/CTMS/int/images/green_check.jpg",2000);
                if (obj.redirect){
                    if(toClone )
                ...
                loadingScreen("Errore clonazione!", "/CTMS/int/images/alerta.gif", 3000);
            }
        },
        error: function(){
            loadingScreen("Errore clonazione!", "/CTMS/int/images/alerta.gif", 3000);
...
function mostraTotCTC(totFlow,totFlowCTC,totTip1,totTip1CTC,totTip2,totTip2CTC  ,totTip3,totTip3CTC,totTip4,totTip4CTC  ,totTip5,totTip5CTC){
    ...
    totalePaziente=totPat;
     ...
     if(pazienti>0){
         var totStudio= (totPat*pazienti)+parseFloat(totTip2)+parseFloat(totTip4)+parseFloat(totTip5);
         var totStudioCTC= (totPatCTC*pazienti)+parseFloat(totTip2CTC)+parseFloat(totTip4CTC)+parseFloat(totTip5CTC);
...
function mostraTotCTCBracci(totFlow,totFlowCTC,totFlowBracci,totFlowBracciCTC,totTip1,totTip1CTC,totTip2,totTip2CTC  ,totTip3,totTip3CTC,totTip4,totTip4CTC  ,totTip5,totTip5CTC){
    ...
    if(isNaN(totTip5)){
     ...
     $.each(docObj.elements.bracci, function(i,braccio){
    	 ...
    	 html+="<tr><td>Budget totale per paziente ("+getDato(braccio.metadata['Base_Nome'])+")</td><td>"+totPatBraccio.formatMoney()+"</td><td>"+totPatBraccioCTC.formatMoney()+"</td></tr>";
     });
    ...
    totalePaziente=totPat;
     ...
     if(pazienti>0){
         ...
         var totStudio= totalAllPatients+parseFloat(totTip2)+parseFloat(totTip4)+parseFloat(totTip5);
         var totStudioCTC= totalAllPatientsCTC+parseFloat(totTip2CTC)+parseFloat(totTip4CTC)+parseFloat(totTip5CTC);
         totStudioCTC=totStudioCTC.toFixed(2);
         totaleStudio= totStudio;
     }
     else{
     ...
     docObj.elements.budgetStudio.metadata['BudgetCTC_TotaleFlowchart']=totFlow.formatMoney().unformatMoney();
 	 ...
 	 docObj.elements.budgetStudio.metadata['BudgetCTC_TotaleStudioCTC']=totStudioCTC.formatMoney().unformatMoney();
         ...
         html+="<tr><td>Budget totale per studio</td><td>"+totStudio.formatMoney()+"</td><td>"+totStudioCTC.formatMoney()+"</td></tr>";
         //html+="<tr><td>Totale SSN</td><td>"+totaleSSN()+"</td><td>N.A.</td></tr>";
	if(isNaN(markup))markup=0;
    $('#table-tot tbody').html(html);
    if(budgetCTC){
...
function calcolaTotali(){
    ...
    if(markup>0 && budgetCTC && false){
		...
		if(targetVisita) {
			newArray[lastRow+2][0] ='Valore target';
			newArray[lastRow+3][0] ='Target markup';
		}
		...
		if(targetVisita) {
			newArray[lastRow+1][0] ='Valore target';
			newArray[lastRow+2][0] ='Target markup';
		}
        for (var ii=1;ii<lastRow;ii++){
    ...
    if(currTarget){
        ...
        if(!isNaN(targetTot) && !isNaN(advisedMarkup)){
        	...
        	if($('#target').val()==3){
        		var targetRow='#table-tot tr:nth-child(2) ';
        	}else{
...
$(function() {
    ...
    $("#dialog-form-cost-2").dialog({
    		...
    		buttons:[
	 		    {
	 				...
	 				click: function() {
	    				var bValid = true;
	    				var tipologia = $("#tipologia2"), descrizione = $("#descrizione2"), costo = $("#costo2"), markupCosto = $("#markup-costo2"), transfer = $("#transfer-costo2"), categoria = $("#interno_esterno"), allFields = $([]).add(tipologia).add(descrizione).add(costo), tips = $(".validateTips");
						var idx=$("#idx").val();
						var update=false;
						if(isNaN(idx) || (!idx && idx!==0))idx=undefined;
						else update=true;
    	...
    	$("#create-cost-2").button().click(function() {
    		$("#dialog-form-cost-2").find(':input').val('');
    		//$("#idx").val('');
    		//$('#tipologia2').val(''); 
        ...
        buttons:[
 		    {
 				text:"Applica", 
 				click:  function() {
 	            	var totPat=parseInt($(this).find('[name=BudgetCTC_NumeroPazienti]').val());
 	            	var bracciTotPat=0;
 	            	var previsti='';
 	            	if(!totPat || isNaN(totPat)){
 	                ...
 	                var totali=calcolaTotali();
 	                pazienti=docObj.elements.budgetStudio.metadata['BudgetCTC_NumeroPazienti']+previsti;
    ...
    $("#add-n-pat").button().click(function() {
        elementToForm(docObj.elements.budgetStudio,'dialog-form-n-pat');
        $.each(docObj.elements.bracci, function(i,braccio){
        	$('#braccioPatients'+i).find('input').val('');
