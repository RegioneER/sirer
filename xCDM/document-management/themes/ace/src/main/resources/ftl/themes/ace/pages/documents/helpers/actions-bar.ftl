  ...
  function DateFmt(fstr) {
			...
			for (var pInst in globalTasks[pKey].instances){
				...
				for (tt=0;tt<tasks.length;tt++){
					...
					if (task.type=="Confirm"){
						...
						$('#task-Actions').append('<li><a class="btn btn-success btn-xs" href="#" onclick="executeConfirmTask('+task.id+');"> <i class="fa fa-play"></i> '+task.name+'</a></li>');						
					}
					if (task.type=="Form"){
						gtasks[task.id]=task;
						console.log("Task di tipo Form");
						$('#task-Actions').append('<li><a class="btn btn-success btn-xs" href="#" onclick="executeFormTask('+task.id+');return false;"><i class="fa fa-pencil"></i> '+task.name+'</a></li> ');						
					}
				}
			}
		}
	...
	$('#bulkFile-form-submit').click(function(){
		...
		<ul class='action-links'>
		    ...
		    <#if !activeProcess>
		    <#if model["availableProcesses"]?? && userPolicy.canLaunchProcess>
		    	...
		    	<ul id="task-Actions"></ul>
			    <#if !el.locked && el.type.checkOutEnabled && userPolicy.canUpdate>
			       <li>
			       		<a href="#" class="btn btn-xs btn-warning checkOut"><i class="fa fa-sign-out"></i> <@msg "actions.doCheckOut"/></a>
				   <li>			    
				</#if>
		        
		    <#if el.locked && el.type.checkOutEnabled && userPolicy.canUpdate && el.lockedFromUser==userDetails.username>
		    	<li>
		    		<a href="#" class="btn btn-xs btn-warning checkIn"><i class="fa fa-sign-in"></i> <@msg "actions.doCheckIn"/></a>
		    	</li>
			<#else>
		        <#if el.locked && el.type.checkOutEnabled && userPolicy.canRemoveCheckOut>
		   		<li>
		   			<a href="#" class="btn btn-xs btn-warning checkIn"><i class="fa fa-unlock"></i> <@msg "actions.forceCheckIn"/></a>
		   		<li>
		        </#if>
		    </#if>
		    <#if (userPolicy.canEnableTemplate && !el.locked) || (userPolicy.canEnableTemplate && el.locked && el.lockedFromUser==userDetails.username)>
		        <#list elType.associatedTemplates as templates>
		            ...
		            <#if !active>
		                <li>
		                <a href="#" id="${templates.metadataTemplate.id}" class="btn btn-xs btn-success addMetadataTemplate"><i class="fa fa-tags"></i> <@msg "actions.addTemplate"/> <@msg "template.${templates.metadataTemplate.name}"/></a>
		                </li>
		            </#if>
		        </#list>
		    </#if>
		    <#if userPolicy.canDelete>
		    	<li>
		        <a href="#" class="btn btn-xs btn-alert delete"><i class="fa fa-trash"></i> <@msg "actions.deleteElement"/></a>
		        </li>
		    </#if>
			<li>
