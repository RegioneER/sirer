...
<@script>
	...
	$('#document-form-submit').closest('.btn').click(function(){
			if ($('#document-form [name]').not('#document-form-submit').length>0) var formData=new FormData($('#document-form')[0]);
			else var formData={};
			...
			bootbox.dialog({
				message:'<i class="icon-spinner icon-spin"></i> <#attempt><@msgJs "js.Saving"/><#recover>Salvataggio in corso</#attempt>',
				backdrop: true,
				closeButton: false
			});
			$.ajax({
