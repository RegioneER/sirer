...
class RepLink {
	...
	function form(){
		...
		$html_form="<form name='link_form' method='post'>
	...
	static function save($data,$conn){
		...
		if(isset($data['TARGET'])){
		    ...
		    if($query->row['CONTO']=='0'){
    		    $str="alter table ".LK_TABLE_LINKS." add target varchar2(1000 CHAR)";
                $query=new query($conn);
                $query->set_sql($str);
		...
		foreach ($data['GRUPPIU'] as $id_gruppo => $abilitato ){
			if($abilitato) RepLink::abilitaGruppo($data['ID'],$id_gruppo,$conn);
			else RepLink::disabilitaGruppo($data['ID'],$id_gruppo,$conn);
		}
		$conn->commit();
	}
	/**
	...
	function getHtml(){
		if($this->config_service['analyzerPatchEnabled'] && $this->data['ID']){
		    if($this->data['HREF']!=''){
                $href= preg_replace ( "/\[(.*?)\]/e", "var_glob('\\1')", $this->data['HREF'] );
            }
    		...
    		return "<div class=\"menu_item\" ><a href=\"{$href}\" target=\"$target\" $fake_link  >".$this->data['TESTO']."</a></div>";
		}
	}
	...
	function getDataLink(){
		if($this->config_service['analyzerPatchEnabled'] && $this->data['ID']){
			...
			return array("TESTO" => $this->data['TESTO'], "HREF" => $href, "TARGET" => $target);
...
class LinkModule{
	...
	function __construct($conn,$xmr){
		...
		$this->update_str=file_get_contents("rep_update_str.sql");
	...
	function controller($session_vars){
      ...
      $msie_css="";
		...
		switch($this->session_vars['REQUEST']){
			case 'LINK_FORM':
				$link=new RepLink($this->conn,$session_vars['ID'],null,$this->config_service['lang'],$this->config_service);
				$form=$link->form();
				$this->result=$form;
				break;
			case 'LINK_DELETE':
                if($this->user->isAmmin()){
				    RepLink::delete($_GET['ID'],$this->conn,$this->user);
                }
				ob_clean();
			...
			case 'SAVE_LINK':
                ...
                if($this->user->isAmmin()){
				    RepLink::save($session_vars,$this->conn);
                }
				break;
			case 'DEFAULT_FORM':
			...
			case 'ABILITA_GRUPPO':
                if($this->user->isAmmin()){
				    $this->result=RepLink::abilitaGruppo($this->session_vars['ID'],$this->session_vars['ID_GRUPPOU'],$this->conn);
                }
				$no_template=true;
				break;
			case 'DISABILITA_GRUPPO':
                if($this->user->isAmmin()){
				    $this->result=RepLink::disabilitaGruppo($this->session_vars['ID'],$this->session_vars['ID_GRUPPOU'],$this->conn);
                }
				$no_template=true;
				break;
			case 'SORT_LINK':
                if($this->user->isAmmin()){
				    RepLink::ordinaLink($this->session_vars['ORDER'],$this->conn);
                }
				echo '{"success":true}';
				die();
				break;
			default:
				...
				/*$this->result="
					...
					addEvent(window, 'load', addListeners);
						...
						<frameset cols=\"".LK_WIDTH."\" frameborder=\"0\">
				   			...
				   			<iframe src=\"$defaultPage\" name=\"main\" id=\"mannaggia\" >
						</frameset>
						</frameset>
				...
				$no_template=true;*/
                $no_template=true;
                if($this->session_vars['REQUEST']=='PREVIEW') $items=$this->user->preview();
                else $items=$this->user->home();
                global $study_;
                //print_r($items);
                $study_->configurer->sidebar = new SideBar($items);
                $this->study_user=$study_->user;
                //Logger::send($this->study_user);
                $this->result="<iframe src=\"$defaultPage\" style=\"width:100%\" class=\"fullHeightContainer\" name=\"main\" id=\"mannaggia\" ></iframe>";
				break;
		}

		if(!$no_template){
			$this->result="<html><head><meta http-equiv='Content-Type' content='text/html; charset=UTF-8' /><link media='screen' href='".LK_CSS."' rel='stylesheet' type='text/css' /><script type=\"text/javascript\" src=\"libs/js/jquery/jquery.min.js\" ></script><script type=\"text/javascript\" src=\"libs/js/jquery/jquery.iphone-switch.js\" ></script><script>$(document).ready(function (){
				...
				$('a[target=main]').click(function(e){
				    ...
				    //top.frames[2].location=$(this).attr('href');//VMAZZEO -- 29.09.2016
				    top.frames[0].location=$(this).attr('href');//VMAZZEO -- bug fixed -- il frame è 0 non 2 (come in vecchia versione)
...
{$this->result}</body></html>";
	...
	function updateStrSave(){
		...
		$this->updateInfo();
		$file_config=fopen("rep_update_str.sql","w");
	...
	function initialize(){
		...
		$table_create[]="create table ".LK_TABLE_AMMINISTRATORI." (
				USERID varchar2(200 CHAR) not null,
		...
		$table_create[]="create table ".LK_TABLE_THEME."
			(
			  CONSTANT    varchar2(200 CHAR) not null,
	...
	function update(){
		...
		$query->get_row();
		if ($query->row['CONTO']==0) {
			$str="alter table ".LK_TABLE_THEME." modify VALUE VARCHAR2(2000 CHAR)";
			$query=new query($this->conn);
			$query->set_sql($str);
			$query->ins_upd();//non richiede binding
...
class AmminUser extends LinkerUser{
	...
	function home(){
		$this->links[]=new RepLink($this->conn,null,array("TIPOLOGIA" => "LINK","TESTO" => Label::testo("Crea un nuovo report",$this->config_service['lang']),"HREF" => "?rep_link=true&REQUEST=LINK_FORM"),$this->config_service['lang'],$this->config_service);
		$this->links[]=new RepLink($this->conn,null,array("TIPOLOGIA" => "LINK","TESTO" => Label::testo("Lista dei link",$this->config_service['lang']),"HREF" => "?rep_link=true&REQUEST=LINK_LIST"),$this->config_service['lang'],$this->config_service);
		$this->links[]=new RepLink($this->conn,null,array("TIPOLOGIA" => "LINK","TESTO" => Label::testo("Default",$this->config_service['lang']),"HREF" => "?rep_link=true&REQUEST=DEFAULT_FORM"),$this->config_service['lang'],$this->config_service);
		$this->links[]=new RepLink($this->conn,null,array("TIPOLOGIA" => "LINK","TESTO" => Label::testo("Aggiornamento dati",$this->config_service['lang']),"HREF" => "?rep_link=true&REQUEST=UPDATE_SETUP"),$this->config_service['lang'],$this->config_service);
		$this->links[]=new RepLink($this->conn,null,array("TIPOLOGIA" => "LINK","TESTO" => Label::testo("Messaggio Utente",$this->config_service['lang']),"HREF" => "?rep_link=true&REQUEST=INFO_FORM"),$this->config_service['lang'],$this->config_service);
		$this->links[]=new RepLink($this->conn,null,array("TIPOLOGIA" => "LINK","TESTO" => Label::testo("Preview",$this->config_service['lang']),"HREF" => "?rep_link=true&REQUEST=PREVIEW","TARGET" => '_self'),$this->config_service['lang'],$this->config_service);
	...
	function preview(){
		$this->links[]=new RepLink($this->conn,null,array("TIPOLOGIA" => "LINK","TESTO" => Label::testo("Esci dalla preview",$this->config_service['lang']),"HREF" => "?rep_link=true","TARGET" => "_self"),$this->config_service['lang'],$this->config_service);
		return $this->getMenu();
	}
}
/**
...
//				    Logger::send($in_link);
			...
			if(count($this->grouppedLinks[$id_link])>0){
				...
				foreach ($this->grouppedLinks[$id_link] as $in_link ){
					...
					if(count($this->grouppedLinks[$in_link->data['ID']])>0){
						$items2=Array();
						foreach ($this->grouppedLinks[$in_link->data['ID']] as $in2_link ){
							$data=$in2_link->getDataLink();
							$items2[]=new SideBarItem(new Link($data['TESTO'], '', $data['HREF'],false,$data['TARGET']));
						}
						
						
					}
					$data=$in_link->getDataLink();
					$items[]=new SideBarItem(new Link($data['TESTO'], '', $data['HREF'],false,$data['TARGET']),$items2);
				}
				$this->result.="</div></div>";
			}
			$data=$curr_link->getDataLink();
			$links[]=new SideBarItem(new Link($data['TESTO'], '', $data['HREF'],false,$data['TARGET']),$items);
		}
		return $links;
	}
	function loadLinks($all=false){
		...
		if($groupsList!=''){
			...
			$query->exec($str);//non richiede binding
			while ($query->get_row()){
				if($query->row['ID_REF']=="")$this->links[$query->row['ID']]=new RepLink($this->conn,null,$query->row,$this->config_service['lang'],$this->config_service);
				else $this->grouppedLinks[$query->row['ID_REF']][$query->row['ID']]=new RepLink($this->conn,null,$query->row,$this->config_service['lang'],$this->config_service);
			}
		}
	}
...
class utenteIanus{
	...
	function getGroups(){
		if(count($this->groups)==0){
			...
			if(count($this->study_user->usersProfiles)>0){
				$str="select ID_GRUPPOU from ".TABLE_UTENTI_GRUPPIU." where userid=:userid and abilitato=1 and id_gruppou not in (select  profile_id from users_profiles where active=0 and userid=:userid)";
