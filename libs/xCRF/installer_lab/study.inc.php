...
class navigation_bar_range {
     ...
     * connessione oracle, al prefisso in banca dati del serviio, alle configurazioni di sistema,
 	 ...
 	 */
 	function navigation_bar_range ( $service=null, $conn=null, $config_service,$vlist){
		...
		$sql_query="
			...
			if ($sql->numrows>0){
				...
				while ($sql->get_row()){
					...
					if (isset($in['CODPAT']) && $in['CODPAT']==$patients[$idx]['CODPAT']){ $selected="selected";$current_subjid=$patients[$idx]['SUBJID'];
					} else { $selected='';}
					$options.="
						<option value=\"{$patients[$idx]['CODPAT']}\" $selected>".($patients[$idx]['SUBJID'] != "" ? $patients[$idx]['SUBJID'] : $patients[$idx]['CODPAT'])."</option>";
				}
				$codice_paziente="
...
//					print_r($in['CENTER']);
					if (isset($in['CENTER']) && $in['CENTER']==$sql->row['CENTER']){ $selected="selected"; 
					}
					else{ $selected='';}
					$options.="
						<option value=\"{$sql->row['CENTER']}\" $selected>{$sql->row['SITEID']}</option>";
				}
				$codice_centro="<b>Site:</b> 
		...
		else{$add_progr_form=$add_progr="&PROGR={$in['PROGR']}";}
		...
		if(isset($in['ESAM']) && isset($in['form'])){
			...
			/*Viene creata la tabella html con i link relativi e assegnata ad una variabile. Tale variabile
		 * sarà l'output del metodo navigation_bar_html($in).*/
			//Azzero la select del paziente
			$codice_paziente = "";
			$nav_bar="
...
//						window.document.location.href=location.href.replace(/CODPAT=(.*?)&/i,'CODPAT='+value+'&');
				...
				function doRedirect2(value) {
					if (location.href.match(/CENTER=/)) {
						window.document.location.href='index.php?&exams=visite_exams.xml&CODPAT='+value+'&CENTER='+value;
...
class Study extends Study_prototype_mxmr { // Utilizzare assolutamente Study_prototype_mxmr e non Study_prototype
	...
	function Controller() {
		...
		if (isset ( $this->session_vars ['ret_registrazione'] ) && $in ['CODPAT'] != '') {
			...
			if (isset ( $this->session_vars ['recovery_password'] )) {
				...
				$body .= "<TABLE align=center class=\"bordi\" width=\"70%\">
	     			<TR>
		           <TD align=center><b><font size=\"3\">The Patient  {$sql_exist->row['SUBJID']} has been registrated</font> <br>
						<br>
						<br>
						<!--<font size=\"4\">PIN CODE:</font>
						<font size=\"4\" color=\"#FF952B\"> {$sql_exist->row['PASSWORD']}</font>-->
					...
					<a href=\"index.php?list=patients_list_Data_Entry.xml\"><u>Registered Patients List</u></a></TD>
					</TR>
				</TABLE>";
			}
	...
	function make_patient_table() {
        ...
        select a.center,
          ...
          from {$this->service}_RANGE_LAB a,{$this->service}_coordinate coor, SITES c
          where 
           c.ID=a.center
							...
							<table class=\"table table-striped table-bordered table-hover\">
								<tr>             
					                <td class=destra >" . mlOut ( "System.patient_table.center_code", "Center Code" ) . "</td>
					               	<td class=input><b> {$sql->row['CODE']}&nbsp;</b></td>
		...
		if ($vlist->esams [$in ['VISITNUM']] [$in ['ESAM']] ['PROGR'] == "yes" && ! isset ( $in ['PROGR'] )) {
			$is_sub_form = true;
		}
...
// 						echo "<br/>lock_obvious ".$lock_obvious;
	 ...
	 */
	function eQBar($lock_query = 0, $lock_obvious = 0) {
		...
		$html_eq = "<div style=\"float:left\"><i class=\"fa fa-comment\"></i>&nbsp;".$link_eq_on."</div>
					<div style=\"float:right\">{$label_eq_not_available}</div><div style=\"clear:both\">&nbsp;</div>
					<div style=\"float:left\"><i class=\"fa fa-pencil-square\"></i>&nbsp;$link_modify_form</div>
					<div style=\"float:right\">{$label_eq_not_available}</div>";
	...
	//	echo $this->config_service ['eQuery']."<br/>";
		if ($in ['USER_TIP'] == 'DE' && $this->config_service ['eQuery'] == '1') {
			...
			if ($lock_query != 1) {
				$html_eq = "<div style=\"float:left\"><i class=\"fa fa-comment\"></i>&nbsp;".$link_eq_on."</div>
									<div style=\"float:right\">{$link_eq}</div>";
			}
		}
