...
function db_query($SQL, $commit = false){
	...
	if ($rbool == false && err_can_show()){
		...
		//die("qua?"); --> TODO: Verificare i messaggi di sistema!
		_db_rollback($c);
		return FALSE;
	}
...
function db_query_bind($SQL, $bind, $commit = false){
	$c = _db_connect();
	$q = db_prefix_tables($SQL);
...
function db_form_updatedb($table, $row, $action, $keycolumn, $commit=true, $notify = true){
	$retval = false;
	if ($notify){
	...
	switch ($action){
		...
		case ACT_MODIFY:
			unset($row['UTENTE']);
			unset($row['AZIENDA']);
			$retval = db_update_row($table,$row,$keycolumn,$commit);
			break;
		default:
			if (err_can_show()){
...
function db_update_row($table,$row,$keycolumn,$commit=true){
	$retval = true;
