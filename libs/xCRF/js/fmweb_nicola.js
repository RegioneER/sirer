...
M[0][23]="\n"+"Si e' usato un codice non permesso.";
M[0][26]=A+"reale <=0";
M[0][27]=A+"reale >=0";
M[0][28]=A+"intero <=0";
M[0][29]=A+"intero >=0";
...
function d(tau) {
	...
	//alert(tau);
	spd=tau.split('###');
	if(is_nat(el[spd[1]+'D'].value) && is_nat(el[spd[1]+'M'].value)){
		el[spd[1]+'D'].value=el[spd[1]+'D'].value.replace(/\s/g,"");
		el[spd[1]+'M'].value=el[spd[1]+'M'].value.replace(/\s/g,"");
		el[spd[1]+'Y'].value=el[spd[1]+'Y'].value.replace(/\s/g,"");
		if(el[spd[1]+'D'].length==1)el[spd[1]+'D'].value='0'+el[spd[1]+'D'].value;
		if(el[spd[1]+'M'].length==1)el[spd[1]+'M'].value='0'+el[spd[1]+'M'].value;
	}
	
	//alert(spd[0]);
	if (is_empty_date(spd[1])) {return 8} //SI ES VACIA
	dd=(el[spd[1]+'D'].value).toUpperCase();
	mm=(el[spd[1]+'M'].value).toUpperCase();
	yy=(el[spd[1]+'Y'].value).toUpperCase();
	if (!yy) {return 17} //SI NO TIENE A�O
	L=spd[0].length;
	tn=spd[0].substring(L-2,L);
	if (!is_nat(yy)&&!is_gc(tn,yy)) {return 25}
	
	if(!is_nat(mm)){ 
	//	alert(el[spd[1]+'M'].value);
		mm=calcularMes(mm);
		if (mm<10) mm='0'+mm;
		}
	...
	if (yy=='CONT'||yy=='NK'||yy=='NA') {
		el[spd[1]+'D'].value='';
		el[spd[1]+'M'].value='';
		if (el[spd[1]] && el[spd[1]+'RC']) { //si existe nombre+RC
			hidden_date(mm,dd,yy,spd[1]) //si existe crea la "fecha absurda"
		}
		return 0;
	}
...
function h(tau) {
	...
	if(is_nat(el[sph[1]+'M'].value) && is_nat(el[sph[1]+'H'].value)){
		el[sph[1]+'M'].value=el[sph[1]+'M'].value.replace(/\s/g,"");
		el[sph[1]+'H'].value=el[sph[1]+'H'].value.replace(/\s/g,"");
		if(el[sph[1]+'M'].length==1)el[sph[1]+'M'].value='0'+el[sph[1]+'M'].value;
		if(el[sph[1]+'H'].length==1)el[sph[1]+'H'].value='0'+el[sph[1]+'H'].value;
	}
...
function jContr(c1,spec) {
	...
	for (j=0;j<trip.length;j++) {
		...
		err=tip[tau.charAt(0)](tau);
		if (err) {jMes_er(err,tau);stop=true;lastErr=err;}
	}
	if(stop) return lastErr;
	return 0;
}
...
function jMes_er(er,tau){
	return mes_er(er,tau,true);
}

function mes_er(er,tau,jquery) {
	...
	if(!jquery)alert(M[_g['L']][50]+des+M[_g['L']][er]+dec+c_p(tn));
	else{
		$.validationEngine.buildPrompt('#'+N,M[_g['L']][50]+des+M[_g['L']][er]+dec+c_p(tn),'error');
	}
	...
	if (er!=13&&er!=3&&er!=36&&er!=38) {
		...
		jst=el[N+z[er2]].type;
		if (jst!='hidden'){
		if(jquery)$.validationEngine.buildPrompt('#'+N+z[er2],M[_g['L']][50]+des+M[_g['L']][er]+dec+c_p(tn),'error');
		...
		if (er==3||er==36) {
			...
			jst=el[N][0].type;
			if (jst!='hidden'){
			if(jquery)$.validationEngine.buildPrompt(el[N][0],M[_g['L']][50]+des+M[_g['L']][er]+dec+c_p(tn),'error');
