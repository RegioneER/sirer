//per resettare tutti i campi di una form
//anche quelli tirati giu dalla banca dati
//E.Gargano 2010
function resetForm() {
			...
			switch(document.forms[i].elements[j].type) {
			  ...
			  case "select-one":
			  		if(document.forms[i].elements[j].selectedIndex!=0)
			  			document.forms[i].elements[j].selectedIndex="";	
			  break;
			 		 
		  	default:
		    	//alert("Elemento non considerato nella funzione resetForm(): "+document.forms[i].elements[j].type)
			}
		}
	}
...
function selall(tipologia,sel) {
	if(sel==1) {
		for (i=0;i<document.forms[0].elements.length;i++) {
			//checkbox_[]_2
			var ghrty="checkbox_[A-Za-z0-9]*_"+tipologia;
			if(document.forms[0].elements[i].name.match(ghrty)) {
...
function check_predef(user,value) {
	var sel='checkbox_'+user+'_'+value;
	if(document.forms[0].elements[sel]) {
		if(document.forms[0].elements[sel].checked==false) {
			alert('Attenzione, il profilo selezionato come predefinito e\' stato abilitato per l\'utente '+user+'.');
			document.forms[0].elements[sel].checked=true;
...
function textCounter(field,counter,maxlimit,linecounter) {
	...
	if (charcnt > maxlimit) { 
		...
		document.getElementById(counter).innerHTML="&nbsp;Limite: "+percentage+"% (-"+(maxlimit-charcnt)+" caratteri)"
...
function apri_window (source,nome) {
	...
	var newwin;
	newwin = window.open(source,'window2', 'width=800,height=425,scrollbars=yes,resizable=yes')
...
if(nome_form.value.match('ï¿½')||nome_form.value.match('ï¿½')){
alert('Attenzione, non ï¿½ possibile utilizzare caratteri speciali come: ï¿½,ï¿½');
	nome_form.focus;
	return false;
}
return true;
...
if(el[nome_form].value.match('ï¿½')||el[nome_form].value.match('ï¿½')){
alert('Attenzione, non ï¿½ possibile utilizzare caratteri speciali come: ï¿½,ï¿½ nel campo '+msg);
	el[nome_form].focus();
	return false;
}
return true;
...
function controlla_data_sys(nome,nomey,nomem,nomed,descrizione)
{
	...
	{	//Costruisco la stringa come append degli elementi
		...
		{			
			...
			{	//La data non ï¿½ nacessaria: tutti gli elementi sono ND
				delete strTest;
				return 0;
			}
			else
...
/*cosï¿½ come sono visualizzate dal browser e sono utilizzate sia	*/
...
{	//controllo la validitï¿½ della prima data
	if (!controlla_data_sys(lowDate,lowDate + 'Y',lowDate + 'M',lowDate + 'D',MsgLowDate))
	{	//controllo la validitï¿½ della seconda data
		...
		{	//Costruisco le date in modo che siano nello stesso formato e con la stessa lunghezza
			...
			{	//higher non deve essere posteriore a lower				
				if	(higher < lower)
				{
					...
					alert ('Attenzione: ' + MsgLowDate +' ('+lowview+')\nnon puï¿½ essere posteriore a ' + MsgHighDate+' ('+highview+')');
			...
			{				
				//alert ('Attenzione: ' + MsgLowDate + ' oppure\n ' + MsgHighDate + ' non ï¿½ valutabile');
				return 0;
			}
		}
		else
		{
			//alert ('Attenzione: '  + MsgHighDate + ' non ï¿½ valida');
	...
	{
		//alert ('Attenzione: ' + MsgLowDate + ' non ï¿½ valida');
		//document.forms[0].elements[lowDate + 'D'].focus();
		return 1;
	}
}
...
/*gg/mm/aaaa, se non puï¿½ costruire una data in questo			*/
...
f=document.forms[0];
el=f.elements;
if(el[cf].type=='hidden') return true;
...
if (el['IMMUNO_12M'][el['IMMUNO_12M'].selectedIndex].value==1)
{
 ...
 if (el['RICADUTE_12M'].value<1) 
 {
 if (!confirm('Il paziente non risulta eleggibile per A\nvalore non corretto nel campo Nï¿½ di ricadute negli ultimi 12 mesi.\nAnnulla per correggere, Ok per accettare')) 
 {
...
if (el['EDSS_ATTUALE'][el['EDSS_ATTUALE'].selectedIndex].value<4) 
{
 ...
 {
 if (!confirm('Il paziente non risulta eleggibile per A\nvalore non corretto nel campo Il paziente ï¿½ stato sottoposto ad un trattamento per almeno 12 mesi con immunomodulanti.\nAnnulla per correggere, Ok per accettare')) 
 {
  el['IMMUNO_12M'].focus();
  return false;
 }
...
//	 alert(fine_date.getDate()+' '+fine_date.getMonth()+' '+fine_date.getFullYear());
  ...
  {
   if (!confirm('La Data di fine della terapia effettuata numero '+i+' non ï¿½ corretta.\nNon sono passati '+numero_gi+' giorni.')) 
   {
...
if (el['DIFFICOLTA_RESPIR'][el['DIFFICOLTA_RESPIR'].selectedIndex].value==2 && el['DIFFICOLTA_RESPIR_PERSIST'].checked)
 {
  alert('Difficoltï¿½ respiratoria Persistente non deve essere selezionato');
...
if (el['ALLERGIA_GRAVE'][el['STANCHEZZA'].selectedIndex].value==2 && el['ALLERGIA_GRAVE_PERSIST'].checked)
 {
  alert('Allergia grave (ipersensibilitï¿½) Persistente non deve essere selezionato');
...
{
var vet_pass_all=new Array();
//<body onload="loadformsdata();getUserid();getFit();//<!--onload-->" onUnload="difformsdata();">	
	
//Gestione uscita di pagina
            //alert ('start');
            var k=0;
            for (k=0;k<document.forms.length;k++){
            //alert(k);
            ...
            var eln=el.length;
                        for (i=0;i<eln;i++) {
                                   ...
                                   tipo=el[i].type;
                                   valore=el[nome].value;
                                   if (nome != 'userid'){vet_pass_all[nome]=valore;}
                        }
            }
return true;
}

function difformsdata()
{
//alert ('ok');
            var k=0;
            for (k=0;k<document.forms.length;k++){
            ...
            var eln=el.length;
            var count_diff=0;
            //var vet_pass_changed=new Array();
                        for (i=0;i<eln;i++) {
                                   nome=el[i].name;
                                   tipo=el[i].type;
                                   valore=el[nome].value;
                                   if (valore != vet_pass_all[nome] && nome != 'userid'){
                                               //&& f.action!=''
                                               //alert(window.action);
                                               //vet_pass_changed[nome]=valore; 
                                               //alert(valore+' '+nome+' '+vet_pass_all[nome]);
                                               count_diff++;
                                               alert('Warning!\nModified Data will be lost!\nIf you are not sure, please go back and verify or save!');
                                               if (tipo!='hidden'){el[nome].focus()}
                                               last;
                                               //return false;
                                   }
                        }
...
/**
 * Funzione libreria:
 * restituisce il parametro della get con il nome fornito
 * 
 * es. http://sito.com?param1=123abc
 * 
 * getParam(param1) --> 123abc
 */
...
/* Funzione libreria:
 * controllare se un valore e' un intero
 * 
 * @param:
 * s: il valore da controllare
 * 
 * @return: true se e' un intero, false altrimenti
 */
...
/* Funzione libreria:
 * controllare se una variabile e' un array
 * 
 * @param:
 * mixed_var: il valore da controllare
 * 
 * @return: true se e' un array valido, false altrimenti
 */
function is_array(mixed_var) {
	...
	//					thus JavaScript objects will also return true
...
/**
 * FUNZIONE PER IL CALCOLO DELLE DIFFERENZE DI DATE.
 *
 * data1 = prima data (in ordine cronologico)
 * data2 = seconda data
 * unita = valori ammessi ('gg' = giorni,
 * 							'mmFULL' = mesi pieni, 
 * 							'mm' = mesi da giorno a giorno
 * 							'aa' = anni)
 * 
 * ***********************************************************************************
 * ************************************************************************************
 * ATTENZIONE:
 * L'opzione mmFULL non si comporta a volte in modo intuitivo:
 * new Date(2010, 0, 1),  // January 1st, 2010; new Date(2010, 2, 12)  // March 12th, 2010 -->Result: 1: February 2010 is the only full month between them
 * 
 * -----------------------------------------
 * opzione mm
 * 
 * 25 Feb - 25 Mar = 1 mese;
 * 25 Mar - 24 Apr = 0 mesi;
 * 
 */
function diff_date(data1, data2, unita){
	difference = 'error';

	if (isNaN(data1.getDate()))
		return "Errore: data1 non valida";
	if(isNaN(data2.getDate()))
		return "Errore: data2 non valida";
	...
	switch(unita){
		case 'gg': 	
					...
					diff = data2 - data1;
					difference = Math.round(diff/(1000*60*60*24));
		...
		case 'aa': 	
					...
					difference = data2.getFullYear() - data1.getFullYear();
					difference = parseInt(difference);
					break;
					
		default: return "Errore, unita non valida";
	}
