...
Control.DatePicker.prototype = {
	initialize: function(element, options) {
		...
		this.options = Object.extend(options || {}, {
				...
				clearValue: this.dateClear.bind(this)
			});
...
Object.extend(Control.DatePickerPanel.prototype, {
	...
	drawCalendar: function(container, date) {
		...
		if (this.options.timePicker) {
			...
			// Aggiunta ritorno a capo fra i due bottoni
			spaceElement = document.createElement('br');
			cell.appendChild(spaceElement);
			
			// Aggiunta bottone clear/sbianca per settare il valore nullo
			clearButton = document.createElement('input');
			clearButton.type = 'button';
			clearButton.value = this.tr('Clear Value');
			clearButton.onclick = function(e) {
						this.options.clearValue(this.currentDate);
					}.bindAsEventListener(this);
