...
function profile_init(){
    ...
    dispatch('/profiles/study/delete_user_profile/:prefix/:username/:profileid', '_delete_user_profile'); //STSANSVIL-2387
...
function _delete_user_profile($prefix, $username, $profileid) {
    $bind['USERID']=$username;
    $bind['STUDY_PREFIX']=$prefix;
    $bind['PROFILE_ID']=$profileid;

    $sql_delete_profile_gruppi = "DELETE FROM UTENTI_GRUPPIU WHERE USERID=:USERID AND ID_GRUPPOU=:PROFILE_ID";
    $sql_delete_profile_users = "DELETE FROM USERS_PROFILES WHERE USERID=:USERID AND PROFILE_ID=:PROFILE_ID";

    $retval = db_query_update_bind($sql_delete_profile_gruppi,$bind,false);

    if($retval) {
        db_commit();
        $retval = db_query_update_bind($sql_delete_profile_users,$bind,false);
        if($retval) {
            db_commit();
        }
    }
