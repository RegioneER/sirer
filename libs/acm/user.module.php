...
function user_form($row, $action){
	...
	$output .= ''.$m->dsp_getCheckbox('ACCESSO SSO ONLY', 'SSO_ONLY', '', '', false, false, $row).'';
...
function user_form_validate(){
	...
	$row = common_booleanCheckBox($row, 'SSO_ONLY');
...
function user_form_submit($action,$old_row=false){
	...
	if (db_form_updatedb($table_user, $vals_user, $action, 'USERID', false)){
		...
		if($action == ACT_INSERT && $row['SSO_ONLY']){//se sono in insert e sono in sso_only sposto di un anno in dietro la scadenza
			$EXPDT="[SYSDATE]-365";
		}
		if($action == ACT_MODIFY && ($row['SSO_ONLY'].""!=$old_row['SSO_ONLY'])){
			//die(var_dump($row['SSO_ONLY']));
			if($row['SSO_ONLY']==1){
				$EXPDT="TO_DATE(DTTM_SCADENZAPWD)-365";
			}
			else if($row['SSO_ONLY']==0){
				$EXPDT="TO_DATE(DTTM_SCADENZAPWD)+365";
			}

			$sql_update_scadenza_pwd = "UPDATE {$table_user} SET DTTM_SCADENZAPWD=".$EXPDT;
			$retval = db_query_update($sql_update_scadenza_pwd, false);
		}
		...
		$vals_user['SSO_ONLY']=$row['SSO_ONLY'];
...
function user_page_view($username){
	...
	//STSANSVIL-2387 aggiunto pulsante per eliminare associazione profilo-utente
	$output .= '&nbsp;<button class="btn btn-danger" onclick="showDeleteProfileUsers(\''.$grid_selector.'\');"  style="width:200px; margin-top:5px;" type="submit" name="delete_user_profile" id="delete_user_profile"><i class="fa fa-trash bigger-110"></i>&nbsp;' . t("Delete selected profile") . '</button>';
...
function user_page_resetcredentials($username){
	...
	$row['DTTM_SCADENZAPWD']='[SYSDATE]+90';//vmazzeo 09/07/2021 aggiorno scadenza a 3 mesi da oggi
