...
function center_init(){
	...
	dispatch('/center/users/delete/:center_id/:user_id','_centers_user_delete');
	...
	dispatch('/center/study/center_assoc_new_user/:centerid/:username/','_center_assoc_new_user');//VAXMR-299
...
function center_users_form_submit($center_id, $list,$row=false,$verbose=true){ //VAXMR-299
	$retval = false;
	if(!$row){
		$row = $_POST;
	}
...
function _centers_user_delete($center_id,$user_id){
	$output="";
	$m = UIManager::getInstance();
	$sql="DELETE FROM USERS_SITES WHERE USERID=:USERID AND SITE_ID=:SITE_ID ";
	$bind=array();
	$bind['USERID']=$user_id;
