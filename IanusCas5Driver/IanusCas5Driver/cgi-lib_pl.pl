...
# if ($ENV{SCRIPT_NAME} ne '/cgi-bin/ossc_save_form'){
# binmode(STDIN,'utf8'); 
#}else{
#binmode(STDIN);	
#}
  ...
  if ($ENV{SCRIPT_NAME} =~ /download/) {binmode(STDOUT);}
  else {
	binmode(STDOUT,'utf8');
...
#  binmode(STDERR);
  ...
  if (!defined $meth || $meth eq '' || $meth eq 'GET' || 
    ...
    foreach $i (0 .. $#in) {
      ...
      use utf8;
      $utf8key=$key = uri_unescape($key);
      $utf8val=$val= uri_unescape($val);
      if (utf8::decode($utf8key)) {$key=$utf8key;}
      if (utf8::decode($utf8val)) {$val=$utf8val;}
