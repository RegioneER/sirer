...
(function(e, g, h, c) {
	...
	var d = {
		...
		maxDepth : 5,
		threshold : 20,
		//vmazzeo gestione clonazione dell'elemento draggato e non indentazione di elementi nella lista dei file
		clonable: false,
		structure:false,
		nestable:true
		//fine vmazzeo gestione clonazione dell'elemento draggato e non indentazione di elementi nella lista dei file
	};
	function i(m, l) {
		this.w = e(g);
		this.el = e(m);
	...
	i.prototype = {
		...
		dragStart : function(q) {
			...
			//INIZIO vmazzeo gestione clonazione dell'elemento draggato
			if(this.options.clonable){

				var cln=o[0].cloneNode(true);
				o[0].parentNode.replaceChild(cln, o[0]);
				$('.dd-handle a').on('mousedown', function(e){e.stopPropagation();}); //risetto la non propagazione del mousdown sui link alle varie opzioni
			}
			else{
				o[0].parentNode.removeChild(o[0]); //vecchia versione che non clona
			}
			//var data_id=o[0].attr("data-id");
			//FINE vmazzeo gestione clonazione dell'elemento draggato
		...
		dragStop : function(m) {
			...
			//INIZIO vmazzeo gestione clonazione dell'elemento draggato
			var dragRootEl_nest=this.dragRootEl.data("nestable").options.nestable;
			if((this.hasNewRoot&&this.options.clonable&&dragRootEl_nest)||(!this.hasNewRoot&&this.options.structure))
			{
				if(this.hasNewRoot){
					//se sposto dalla lista alla struttura allora devo cambiare l'id del div
					var max_id=0;
					var my_type=l.attr('data-id').substring(0,1);
					$("#study_structure div[id^='handler_"+my_type+"_']").each(function(){
						var this_id=parseInt($(this).attr('id').substring($(this).attr('id').lastIndexOf('_')+1));
						//alert(this_id);
						if(this_id>max_id){
							max_id=this_id;
						}
					...
					max_id++;
					l.attr('data-id',my_type+'_'+max_id);
					nome_xml=l.children($('.class dd-handle')).attr('id').substring(10);
					l.children($('.class dd-handle')).attr('id','handler_'+my_type+'_'+max_id );
					var handler=l.children($('.class dd-handle'));
					
					handler.children(".action-buttons").each(function(){
						if($(this).css("display")=="none"){
						    $(this).css("display","block");
						    $(this).children("a").attr('id','a_'+max_id );
						    $(this).children("input").attr('id','H_'+my_type+'_'+max_id); //modifico anche l'id dell'hidden
						    //alert( $(this).children("input").attr('id') );
						    var my_val=$(this).children("input").attr('value'); 
						    if(my_val!="{}"){//se ho già il campo valorizzato allora aggiungo number e xml ai valori che ho già
						    	$(this).children("input").attr('value','{"number":"'+max_id+'","xml":"'+nome_xml+'"'+my_val.replace("{",","));
						    } 
						    else{
						    	$(this).children("input").attr('value','{"number":"'+max_id+'"}');
						    }
						}
						else{
						    $(this).remove();
				...
				/*$('#handler_'+my_type+'_'+max_id+' .action-buttons').each(function(){
					if($(this).css("display")=="none"){
					    $(this).css("display","block");
					}
					else{
		...
		dragMove : function(s) {
			...
			if (u.dirAx && u.distAxX >= m.threshold) {
				...
				if (u.distX > 0 && n.length && !n.hasClass(m.collapsedClass)) {
					...
					//vmazzeo non indentazione di elementi nella lista dei file
					var dragRootEl_nest=this.dragRootEl.data("nestable").options.nestable;
					if (dragRootEl_nest && (p + this.dragDepth <= m.maxDepth)) {
