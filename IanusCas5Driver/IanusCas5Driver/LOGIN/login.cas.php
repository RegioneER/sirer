...
if (file_exists($_SERVER['DOCUMENT_ROOT']."/authzssl/login_template_cas.html")) $templateFile=$_SERVER['DOCUMENT_ROOT']."/authzssl/login_template_cas.html";
else $templateFile="/http/lib/IanusCas5Driver/LOGIN/template/login_template.html";
...
$commonMessageFile="/http/lib/IanusCas5Driver/LOGIN/messages_{$language}.inc";
$defaultMessageFile="/http/lib/IanusCas5Driver/LOGIN/messages_en.inc"; //Questa è la lingua di default!
...
if ($_SERVER['DOCUMENT_ROOT']."/authzssl/login-copyright.html") {
	...
	$copyright_file_content=file_get_contents("/http/lib/IanusCas5Driver/LOGIN/login-copyright.html");
}
...
//die("AAA");	
	...
	$cssCookie='<link rel="stylesheet" type="text/css" href="/authzssl/cookieConsent.css"/>';
	...
	$form=file_get_contents("/http/lib/IanusCas5Driver/LOGIN/template/login_form.html");
	$form=str_replace("__service__", $_POST['service'], $form);
	$form=str_replace("__execution__", $_POST['execution'], $form);
	...
	if (isset($_POST['loginError']) && $_POST['loginError']=='true'){
	...
	require_once '/http/lib/IanusCas5Driver/ApacheCrypt.inc.php';
	//$_POST['password']=strtoupper($_POST['password']);
	$ret=CheckPassword($_POST['username'], $_POST['password']);
	
	if ($ret['userType']=='SSO') $sso=true;
	else $sso=false;
	
	if (!$sso) $_POST['username'].="@".$_SERVER['HTTP_HOST'];
	
	if ($ret['preauthSuccess']){
	
		$form="<form id='login_form' method='post' name='login' action='https://sirer-cas.sissprep.cineca.it/cas/login' enctype='application/x-www-form-urlencoded'>";
		...
		$form="<form id='login_form' method='post' name='login' action='login.php' enctype='application/x-www-form-urlencoded'>";
		foreach ($_POST as $key=>$val){
			if ($key=='submit') $form.="<noscript><input type='submit' name='_{$key}' value='$val'/></noscript>";
			else $form.="<input type='hidden' name='$key' value='$val'/>";
		}
		$form.="<input type='hidden' name='loginError' value='true'/>";
		$form.="</form>";
		$loading="<div style=\"padding:4px\"><h2 style=\"padding-right:60px;float:right;\">".$messages['__processing__']."...</h2><i class=\"fa fa-spinner fa-spin fa-4x\"></i> </div>";
		$content=$form.$loading;
...
function CheckPassword($userid, $password){
	...
	$cas5Conn=new LoginDBConnection(); //"GARAER", "gra3r24!01", "(DESCRIPTION=(ADDRESS_LIST=(LOAD_BALANCE=ON)(FAILOVER=ON)(ADDRESS=(PROTOCOL=tcp)(HOST=cman01-ext.dbc.cineca.it)(PORT=5555))(ADDRESS=(PROTOCOL=tcp)(HOST=cman02-ext.dbc.cineca.it)(PORT=5555)))(CONNECT_DATA=(SERVICE_NAME=db41_ext.pdv.cineca.it)))");
	$cas5Sql=new LoginSql($cas5Conn);
	
	$ret['userExists']=false;
	$ret['preauthSuccess']=false;
	$ret['userType']="LOCAL";
	
	$username=$userid;//."@".$_SERVER['SERVER_NAME'];
	
	//$sql_query_sso="select * from CAS5_USERS where upper(username)=upper(:username)";
	//$bind=[];
	//$bind['username']=$username;
	//$cas5Sql->Exec($sql_query_sso, $bind);
	
	//if ($cas5Sql->getRow()){
	//	$ret['userType']="SSO";
	//	$ret['userExists']=true;
	//	$db_password=$cas5Sql->row['PASSWORD'];
	//}else {
		$sql_query_sso="select * from CAS5_USERS where upper(username)=upper(:username)";
		$bind=[];
		$username=$userid."@".$_SERVER['SERVER_NAME'];
...
//die();
	...
	if ($ret['userExists']){
		if ($password==$db_password || crypt($password,substr($password, 0,2))==$db_password || MD5AdminMd5Crypt($caseSensitivePassword, $seed)==$db_password || MD5AdminMd5Crypt($password, $seed)==$db_password){
			$bCryptedPass= password_hash($password, PASSWORD_BCRYPT, array("cost" => 5));
			$updateCas5="update CAS5_USERS set password=:password where upper(username)=upper(:username)";
			$bind=[];
			$bind['username']=$username;
			$bCryptedPass=str_replace('$2y$', '$2a$', $bCryptedPass);
			$bind['password']=$bCryptedPass;
			$cas5Sql->Exec($updateCas5, $bind);
			$cas5Conn->commit();
			$ret['preauthSuccess']=true;
...
class LoginDBConnection {
	...
	public function __construct($user=null, $pass=null, $host=null){
		...
		//die($ammin_file);
		try {
			$this->resource=oci_connect($user, $pass, $host) or die("<h1>Problema con Certificati SSLAAA</h1> <a href=\"javascript: history.go(-1);\" >effettuare nuovamente il login</a>");
		} catch (Exception $e) {
