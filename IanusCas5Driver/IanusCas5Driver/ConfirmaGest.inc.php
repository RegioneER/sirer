...
class ConfirmaGest{
	...
	function ConfirmaGest(){
		...
		else if (preg_match("!\.sissprep\.!",$_SERVER['HTTP_HOST']))
		$this->url_tomcat="http://appserv-siss.pp.cineca.it/ConfirmaSISS/WS?wsdl";
		else $this->url_tomcat="https://appserv-siss.cineca.it/ConfirmaSISS/WS?wsdl";
	}

	function TransactionExists($servizio, $id){
		$tid="{$servizio}_{$id}";
		if(preg_match("/\.sissdev\./", $_SERVER['HTTP_HOST']) || preg_match("/\.sissprep\./", $_SERVER['HTTP_HOST'])){
