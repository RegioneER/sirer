        ...
        $(td).off('dblclick').on('dblclick',function(ev){
            ...
            $("#dialog-form-transfer").off('dialogopen').on('dialogopen',function(ev){
                ...
                $(this).parent().find('button:contains(Aggiungi)').off('click').on('click',function(){
                	var numeric;
	                /*numeric=$(that).find('[name=Costo_OreUomo]').val();
	                if(numeric!='' && isNaN(parseFloat(numeric))){
	                	bootbox.alert('Attenzione inserire un valore numerico nel campo "Ore uomo"');
	                	return false;
	                }*/
	                
	                /*numeric=$(that).find('[name=Costo_OreMacchina]').val();
	                if(numeric!='' && isNaN(parseFloat(numeric))){
	                	bootbox.alert('Attenzione inserire un valore numerico nel campo "Ore macchina"');
	                	return false;
	                }*/
                    ...
                    var newValue='';
                    myElement=formToElement('dialog-form-transfer',myElement);
                    newValue=$.axmr.guid(myElement,getDato(myElement.metadata['tp-p_Checked']),getDato(myElement.metadata['Costo_TransferPrice']));         
                ...
                $(this).parent().find('button:contains(Applica)').off('click').on('click',function(){
                    ...
                    var rowData=$('#costi').handsontable('getDataAtRow',row);
                    for(var idx=1;idx<rowData.length;idx++){
                    	var currCol=idx+1;
	                    var newValue='';
	                    myElement=$.axmr.guid(rowData[idx]);
	                    console.log("pre eseguo",rowData);
	                    if($.isPlainObject(myElement) && myElement.metadata && myElement.metadata['Rimborso_Rimborsabilita']!=2){
		                    
		                    myElement=formToElement('dialog-form-transfer',myElement);
		                    console.log("Eseguo",rowData[idx],myElement,row,idx);
		                    newValue=$.axmr.guid(myElement,getDato(myElement.metadata['tp-p_Checked']),getDato(myElement.metadata['Costo_TransferPrice']));         
		                    $('#costi').handsontable('setDataAtCell',row,idx,newValue);
	                    }
