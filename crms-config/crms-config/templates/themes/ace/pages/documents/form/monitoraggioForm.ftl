...
<#assign budgetType=model['parent'].getFieldDataString("statoValidazioneCentro","typeBudgetApproved") />
<#assign budgetId=model['parent'].getFieldDataString("statoValidazioneCentro","idBudgetApproved") />
						...
						<#if assocTemplate.enabled && templatePolicy.canCreate>
							...
							<#if template.fields??>
				                            <#list template.fields as field>
				                                    <#if field.mandatory && (budgetType!="Budget" || field.name!="BraccioBudget")>
														...
														date_split1=$('#DatiMonitoraggioAmministrativo_periodoRiferimentoDa').val().split("/");
														date1= new Date(date_split1[2], date_split1[1]-1, date_split1[0], 0, 0, 0, 0);
														date_split2=$('#DatiMonitoraggioAmministrativo_periodoRiferimentoA').val().split("/");
														date2= new Date(date_split2[2], date_split2[1]-1, date_split2[0], 0, 0, 0, 0);
														if (date2<date1){
															alert ('la data di riferimento della rilevazione Dal non può essere precedente alla data di riferimento della rilevazione Al');
															return false;
														}
				                                    </#if>
				                            </#list>
		...
		<#if assocTemplate.enabled && templatePolicy.canCreate>
			...
			<#if template.fields??>
                            <#list template.fields as field>
                            	...
                            	<#if (budgetType!="Budget" && budgetType!="") || field.name!="BraccioBudget" >
	                                  <div class="form-group field-component field-editable" id="file_data">
	                                    <@mdfield template field/>
	                             </div>
	                             <#elseif field.name=="BraccioBudget" >
	                             	<input type="hidden" name="DatiMonitoraggioAmministrativo_BraccioBudget"  id="DatiMonitoraggioAmministrativo_BraccioBudget" value="${budgetId}###Singolo braccio" />
	                             </#if>
                             </#list>
                         </#if>
