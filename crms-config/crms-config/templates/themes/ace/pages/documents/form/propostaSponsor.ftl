...
$(document).ready(function(){
 	$.ajax({
  type: "GET",
  url: baseUrl+"/app/rest/documents/advancedSearchCount/PropostaSponsor",
  data: "parent_obj_id_eq=${model['parent'].getId()}",
 	async:false,
  ...
  success: function(dato){
     if(dato=="0"){
    	...
    	$('#document-form-submit').remove();
    	bootbox.alert('Prima proposta promotore gi� inserita.');
    	check_insert=false;
  ...
  success: function(dato){
     if(dato=="0"){
    	...
    	alert('Prima proposta promotore gi� inserita.');
    	check_insert=false;
    	window.location.href='${baseUrl}/app/documents/detail/${model['parent'].getId()}<#if model['docDefinition'].hashBack?? >#${model['docDefinition'].hashBack}</#if>';
    }
  }
