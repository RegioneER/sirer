<@script>
		$(document).ready(function(){
			$('#prodottoStudio-prodottoStudio_DoiPmid').parent().hide();
			$('#prodottoStudio-prodottoStudio_ISBN').parent().hide();
			$('#prodottoStudio-prodottoStudio_codiceBrevetto').parent().hide();

		});

		$('#prodottoStudio_tipoProdotto-select').on('change',function(){
			if($('#prodottoStudio_tipoProdotto-select').val().startsWith('1###')){
				$('#prodottoStudio-prodottoStudio_DoiPmid').parent().show();
				$('#prodottoStudio-prodottoStudio_ISBN').parent().hide();
				$('#prodottoStudio-prodottoStudio_ISBN').val("");
				$('#prodottoStudio-prodottoStudio_codiceBrevetto').parent().hide();
				$('#prodottoStudio-prodottoStudio_codiceBrevetto').val("");
			}
			else if($('#prodottoStudio_tipoProdotto-select').val().startsWith('2###')){
				$('#prodottoStudio-prodottoStudio_DoiPmid').parent().hide();
				$('#prodottoStudio-prodottoStudio_DoiPmid').val("");
				$('#prodottoStudio-prodottoStudio_ISBN').parent().show();
				$('#prodottoStudio-prodottoStudio_codiceBrevetto').parent().hide();
				$('#prodottoStudio-prodottoStudio_codiceBrevetto').val("");
			}
			else if($('#prodottoStudio_tipoProdotto-select').val().startsWith('7###')){
				$('#prodottoStudio-prodottoStudio_DoiPmid').parent().hide();
				$('#prodottoStudio-prodottoStudio_DoiPmid').val("");
				$('#prodottoStudio-prodottoStudio_ISBN').parent().hide();
				$('#prodottoStudio-prodottoStudio_ISBN').val("");
				$('#prodottoStudio-prodottoStudio_codiceBrevetto').parent().show();
			}
			else{
				$('#prodottoStudio-prodottoStudio_DoiPmid').parent().hide();
				$('#prodottoStudio-prodottoStudio_DoiPmid').val("");
	...
	$('#document-form-submit').ready(function() {
		$('#DatiEmendamento_inviatoEme').parent().parent().hide();
	});
...
<form class="form-horizontal" id="document-form" method="POST" action="${baseUrl}/app/rest/documents/save/${model['docDefinition'].id}" enctype="multipart/form-data">
    <h2><@msg "type.create."+model['docDefinition'].typeId/></h2>
        ...
        <@hidden "parentId" "parentId" model['parentId']/>
        	...
        	<#assign elStudio=model['parent'] />
        		<#assign idStud=elStudio.getfieldData("UniqueIdStudio","id")[0] />
        			<@hidden "idStud" "idStud" idStud/>
        ...
        <#if model['docDefinition'].hasFileAttached>
        <#if !model['docDefinition'].noFileinfo>
        <div class="form-group field-component" id="file_version">
        ...
        <button class="btn btn-warning submitButton round-button blue" type="button" id="document-form-submit" name="document-form-submit"><i class="icon-save bigger-160"></i><b>Salva</b></span>
    </fieldset>
