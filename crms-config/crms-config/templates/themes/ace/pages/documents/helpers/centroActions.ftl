...
<fieldset>
    ...
    <#if (userPolicy.canEnableTemplate && !el.locked) || (userPolicy.canEnableTemplate && el.locked && el.lockedFromUser==userDetails.username)>
        <#list elType.associatedTemplates as templates>
            ...
            <#if !active>
            <#if templates.metadataTemplate.name="DatiCentro">
                <input id="${templates.metadataTemplate.id}" class="submitButton round-button blue addMetadataTemplate" type="button" value="Aggiungi informazioni centro-specifiche"><br/>
            <#else>
