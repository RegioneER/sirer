<@script>
	...
	function loadTasks(){
		...
		<#if !(budget.getFieldDataStrings("BudgetCTC","Definitivo")?? && budget.getFieldDataStrings("BudgetCTC","Definitivo")?size>0 && budget.getFieldDataStrings("BudgetCTC","Definitivo")[0]=="1") >
		
		$.ajax({
			type: "GET",
			url: "${baseUrl}/app/rest/documents/tasks/${budget.id}",
			dataType: "json",
			success: function(tasks){
			...
			for (var pInst in globalTasks[pKey].instances){
				...
				for (tt=0;tt<tasks.length;tt++){
					...
					if (task.type=="Confirm"){
						...
						var furtherClass="first";//TOSCANA-135 inizio
						if(task.name=="Chiudi budget studio"){
						    furtherClass="third";
						}
						else if(task.name=="Considera budget dei prezzi come finale"){
                            furtherClass="second";
                        } //TOSCANA-135 fine
						$('#task-Actions').append('<button class="btn btn-primary '+furtherClass+'" style="float:right;" onclick="executeConfirmTask('+task.id+');">'+task.name+'</button>');						
					}
					if (task.type=="Form"){
						gtasks[task.id]=task;
						console.log("Task di tipo Confirm");
