...
td[id^=td-label-DatiAdempimentiAmm]{
width: 80% !important;
}

td[id^=td-field-DatiAdempimentiAmm]{
width: 20% !important;
}
...
<div id="processes">
	...
	<#--GC 01/02/2015 - Mostro il bottone del processo di valutazione POST CESC se l'istruttoria NRC è positiva--> 
	<#assign ValRespIstr="" />
...
<#list el.templates as template>
	<#if template.name="DatiAdempimentiAmmGA" || template.name="DatiAdempimentiAmmME">

	<fieldset class="vs">
		<legend>Adempimenti Amministrativi <@elementTitle el/></legend>
	</fieldset>
	    				...
	    				<#if editable && (templatePolicy.canCreate || templatePolicy.canUpdate)>
      					...
      					<form name="${template.name}" style="display:" id="form-${template.name}" method="POST" action="${baseUrl}/app/rest/documents/update/" onsubmit="return false;">
							...
							<table class="table table-bordered" id="checklist">
								<thead>
									<tr><th colspan="2">Attivit&agrave; di valutazione POST CESC</th></tr>
								</thead>
							<tbody>
