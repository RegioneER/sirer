				...
				<div class="col-xs-12">
    				...
    				<table id="sample-table-1" class="table table-striped table-bordered table-hover">
    					...
    					<#list parentEl.getChildrenByType("AllegatoEme") as subEl>
							<#-- TODO: Inserire controllo centro specifico -->
							<#assign CentroOBJID=(subEl.getFieldDataCode("AllegatoEme","allegatoCentri"))![] />
							<#assign strutturaCODE="0" />
							<#-- IdCentro_Struttura -->
							<!--
							Struttura CODE: ${CentroOBJID}<br/>
							-->
							...
							<#if true> <#-- userHasSite-->
								...
								<#assign uploadDt="" />
								<#assign data="" />
								<#if subEl.getfieldData("AllegatoEme","TipoDocumento")?? && subEl.getfieldData("AllegatoEme","TipoDocumento")[0]??>
								...
								<tr>
									...
									<td>
										<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
											...
											<#if userDetails.hasRole("CTC") || userDetails.hasRole("SEGRETERIA")>
    	...
    	<div class="row">
				<div class="col-xs-12">
    				...
    				<table id="sample-table-1" class="table table-striped table-bordered table-hover">
							...
							<#assign noDoc>
								<tr>
	            		<td colspan=8><span class="help-button">?</span> Nessuna istruttoria inserita</td>
	           		</tr>
           		</#assign>
    					...
    					<#list parentEl.getChildrenByType("IstruttoriaEme") as subEl>
								...
								<#assign uploadDt="" />
								<#assign data="" />
								<#if subEl.getfieldData("IstruttoriaEme","TipoDocumento")?? && subEl.getfieldData("IstruttoriaEme","TipoDocumento")[0]??>
								...
								<#if userDetails.hasRole("SEGRETERIA") || ( subEl.getFieldDataString("IstruttoriaEme","istruttoriaWFinviata")?? && subEl.getFieldDataString("IstruttoriaEme","istruttoriaWFinviata")=="1")>
								<tr>
									<#assign dataRicDoc=""/>
									<#if subEl.getFieldDataDate("IstruttoriaEme","DataRicDoc")?? >
										<#assign dataRicDoc=subEl.getFieldDataDate("IstruttoriaEme","DataRicDoc").time?date?string.short />
									</#if>
									<td><a class="center-link" href="${baseUrl}/app/documents/detail/${subEl.id}"> ${dataRicDoc}</a></td>
    	...
    	<div class="row">
				<div class="col-xs-12">
    				...
    				<table id="sample-table-1" class="table table-striped table-bordered table-hover">
							...
							<#assign noDoc>
								<tr>
	            		<td colspan=8><span class="help-button">?</span> Nessun parere inserito </td>
	           		</tr>
           		</#assign>
    					...
    					<#list parentEl.getChildrenByType("ParereEme") as subEl>
							...
							<#if userDetails.hasRole("SEGRETERIA") || subEl.getFieldDataString("ParereEme","ParereWFinviato")=="1">
								<tr>
									<#if subEl.getFieldDataDate("ParereEme","dataSeduta")??>
									...
									<td>
										...
										<#if !(subEl.getFieldDataString("ParereEme","ParereWFinviato")=="1")>
											<button title="Attiva modifiche" class="btn btn-info btn-xs" style="margin-top:0px"
													data-eme-key-id="${parentEl.id}_${subEl.getFieldDataString('ParereEme','CentroEmeId')}"
													data-eme-id="${parentEl.id}" data-centro-id="${subEl.getFieldDataString('ParereEme','CentroEmeId')}">
												<!--a hidefocus="hidefocus" class="btn-link" role="button" title="Mostra modifiche emendamento" style="" href="#"
												data-eme-key-id="${parentEl.id}_${subEl.getFieldDataString('ParereEme','CentroEmeId')}"
												data-eme-id="${parentEl.id}" data-centro-id="${subEl.getFieldDataString('ParereEme','CentroEmeId')}"-->
												<i class="icon-time"></i> Modifiche pending
												<!--/a-->
											</button>
											<@script>
											$('button[data-eme-key-id="${parentEl.id}_${subEl.getFieldDataString('ParereEme','CentroEmeId')}"]').unbind('click');
											$('button[data-eme-key-id="${parentEl.id}_${subEl.getFieldDataString('ParereEme','CentroEmeId')}"]').click(function(){
											showEmeData($(this).attr("data-eme-id"), $(this).attr("data-centro-id"));
											});
											</@script>
