			...
			$.ajax({
				...
				success: function(obj){
					if (obj.result=="OK") {
						...
						if (obj.redirect){
							window.location.href='${baseUrl}/app/documents/detail/${el.getId()}';
						}
					}
					else{
						bootbox.hideAll();
		...
		<div class="well">
			...
			<span>
		            ...
		            <#if el.file?? && ! elType.noFileinfo>
					    ...
					    <#--
				        ${prevVersion!""}</b><br/>
				        <#assign prevVersion=prevVersion-1/>
				        	-->
				        ${el.file.version!""}
				        <#if el.file.date??>
...
<#if !el.file?? && (userPolicy.canUpdate && !el.locked) || (userPolicy.canUpdate && el.locked && el.lockedFromUser==userDetails.username)>
<!--button class="submitButton btn btn-primary" type="button" id="attachFile-form-showDialog" name="document-form-submit">
	<i class="icon-cloud-upload"></i> Carica Dichiarazione di partecipazione allo studio
</button><br/-->

<!--div id="attachFile-dialog" title="Carica Dichiarazione di partecipazione allo studio"-->
	<form id="attachFile-form" method="POST" action="${baseUrl}/app/rest/documents/${el.id}/attach" enctype="multipart/form-data">
		<#if ! elType.noFileinfo>
			...
			<div class="field-component" id="data-div">
				<label class="col-sm-3 control-label no-padding-right" for="data">Data documento:</label>
				<div class="col-sm-4">
...
<div id="task-Actions" style="text-align: left"></div>
</#if>
</div>
</div>
