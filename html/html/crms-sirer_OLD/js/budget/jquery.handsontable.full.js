 ...
 *   * @param {Number} [amount] Optional.
  ...
  function applyChanges(changes, source) {
    ...
    instance.forceFullRender = true; //used when data was changed
    //G.Delsignore modifica per migliorare performance
    if(renderTimeout){
    	clearTimeout(renderTimeout);
    }
    renderTimeout=clearTimeout(function(){
    	grid.adjustRowsAndCols();
        selection.refreshBorders(null, true);
    },500);
    //G.Delsignore fine modifica per migliorare performance
...
Handsontable.TimePoint = {
  editor: Handsontable.TimePointEditor,
  renderer: Handsontable.TimePointRenderer
};
  ...
  function HandsontableAutoColumnSize() {
    ...
    this.determineColumnWidth = function (col) {
      ...
      for (var r = 0; r < rows; r++) {
      	...
      	var cellValue=instance.getDataAtCell(r, col);
      	var label1=$.axmr.label(cellValue);
      	var label2=$.axmr.label(cellValue,2);
      	if(label1 && label2){
      		cellValue=(label1.length>label2.length)?label1:label2;
      	}
      	else if(label1 || label2 ){
      		cellValue=label1?label1:label2;
      	}
        var value = Handsontable.helper.stringify(cellValue);
        //fine modifiche
