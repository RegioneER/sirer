...
var folderTpxp=null;
var folderCostiAggiuntivi=null;
var folderPxp=null;
var folderPxs=null;
...
$('[name=Prestazioni_Attivita][value^=3]').closest('.radio').hide();
$('[name=Prestazioni_Attivita][value^=4]').closest('.radio').hide();
function changeTipologia(){
	...
	if(tipologia==4){
		...
		//$('[name=Prestazioni_Attivita][value^=3]').closest('.radio').show();
		//$('[name=Prestazioni_Attivita][value^=4]').closest('.radio').show();
	}else if(tipologia==5){
		$('#Prestazioni-Prestazioni_Attivita').hide();
		$('[name=Prestazioni_Attivita][value^=1]').closest('.radio').hide();
		$('[name=Prestazioni_Attivita][value^=2]').closest('.radio').hide();
		//$('[name=Prestazioni_Attivita][value^=3]').closest('.radio').hide();
		//$('[name=Prestazioni_Attivita][value^=4]').closest('.radio').hide();
	}
	else{
	...
	if(tipologia==1 || true){
		...
		//$( "#Prestazioni_CDC2" ).off('click');
		//$( "#descrizione" ).off('change');
		//$( "#Prestazioni_CDC2" ).autocomplete();
		//$( "#descrizione" ).autocomplete();
		//$( "#Prestazioni_CDC2" ).autocomplete('destroy');
		//$( "#descrizione" ).autocomplete('destroy');
		//$('.tip1_exclusive').hide();
	}
}
...
function buildTpDescription(tp){
    ...
    if(metadata['TimePoint_Descrizione']) description=metadata['TimePoint_Descrizione']+'\n';
     ...
     if(metadata['TimePoint_Note'] )description+="\nNote* "    ;
...
function mostraTotCTC(totFlow,totFlowCTC,totTip1,totTip1CTC,totTip2,totTip2CTC  ,totTip3,totTip3CTC,totTip4,totTip4CTC  ,totTip5,totTip5CTC){
    ...
    totalePaziente=totPat;
     ...
     var html='';
         ...
         html+="<tr><td>Budget totale per paziente</td><td></td><td>"+totPat.formatMoney()+"</td><td>"+totPatCTC.formatMoney()+"</td><td></td</tr>";
    ...
    /*$('#markup-ins1').html(markup1+'%');
    $('#markup-ins2').html(markup2+'%');*/
}
...
function updatePrezzi(markup){
	...
	for(var i = 1; i < newArray.length; i++) {
        ...
        for(var k = 1; k < newArray[i].length; k++) {
            ...
            var isPIElement=false;
            

            if($.isPlainObject(currElement)){
            	/*if(getDato(getDato(getDato(currElement.metadata['PrezzoFinale_Prestazione']).metadata['tp-p_Prestazione']).metadata['Prestazioni_CDC'])=="PI"){
            		isPIElement=true;
            	}*/
            	if(true || isPIElement){
	            	...
	            	value=value.formatMoney().unformatMoney();
	            	if(!getDato(currElement.metadata['PrezzoFinale_Prezzo'])) {
	            		currElement.metadata['PrezzoFinale_Prezzo']=value;
    ...
    $.each(docObj.elements.prezzi,function(i,prezzo){
    	...
    	switch(type){
    		...
    		case 'PrestazioneXPaziente':
    			var isPIElement=false;
    			/*if(getDato(getDato(prezzo.metadata['PrezzoFinale_Prestazione']).metadata['Prestazioni_CDC'])=="PI"){
            		isPIElement=true;
            	}*/
    			if(true || isPIElement){
	    			var value=getDato(getDato(prezzo.metadata['PrezzoFinale_Prestazione']).metadata['Costo_TransferPrice'])-0;
	    			var quantita=getDato(getDato(prezzo.metadata['PrezzoFinale_Prestazione']).metadata['Costo_Quantita'])-0;
	    			value=value+(value*markup/100);
	    			value=value.formatMoney().unformatMoney();
...
function calcolaTotali(){
    ...
    var newArray = clone(array);
    var totCols = new Array();
    var totPrezzoCols = new Array();
    var totRows = new Array();
    var totPrezzoRows = new Array();
    var totCostiRows = new Array();
    // vmazzeo 02.05.2017 HCRPMS-461 - inizio
    var totCostiRowsA = new Array();
    var totCostiRowsB = new Array();
    var totCostiRowsC = new Array();
    var totCostiRowsD = new Array();
    // vmazzeo 02.05.2017 HCRPMS-461 - fine
    var totCountSSNRows = new Array();
    var totCountExtraRows = new Array();
    var totCountExtraRowsA = new Array();
    var totCountExtraRowsB = new Array();
    var totCountExtraRowsC = new Array();
    var totCountExtraRowsD = new Array();
    var totCounts = new Array(); //totCounts dovrebbe essere  totCountsRows
    // vmazzeo 23.09.2016 - inizio
    var totCostiCols = new Array();
    var totCountSSNCols = new Array();
    var totCountExtraCols = new Array();
    var totCountsCols = new Array();
    // vmazzeo 23.09.2016 - fine
    var Total = 0;
    var lastCol = 0;
    var lastRow = 0;
    var lastRowFound=false;
    ...
    for(var i = 1; i < newArray.length; i++) {
        ...
        var totCount = 0; //totCount dovrebbe essere  totCountRow
        var currentPrezzoCell = 0;
        ...
        for(var k = 1; k < newArray[i].length; k++) {
            ...
            if((k<lastCol || lastCol==0) && (i<lastRow || lastRow==0)){
                if(value && value != 'false') {
                    ...
                    if(!isNaN(value)){
                    	totRows[i] += parseFloat(value);
                    	totPrezzoRows[i] += parseFloat(currentPrezzoCell);
                    	var costo=parseFloat(getDato(currElement.metadata['Costo_Costo']));
                    	if(!isNaN(costo)){
                    		totCostiRows[i] += parseFloat(getDato(currElement.metadata['Costo_Costo']));
                    	}
                    	if(!totCountsCols[k]){
                    		totCountsCols[k] = 0; 
                    	}
	                	...
	                	if(currElement && getDato(currElement.metadata['Rimborso_Rimborsabilita'])!='2') {
	                     	...
	                     	totCountExtraRow++;
	                     	var copertura=getDato(currElement.metadata['Costo_Copertura']);
	                     	if(copertura){
		                     	if(copertura.match("^1###")){
		                     		totCountExtraRowA++;
		                     		totCostiRowA+=parseFloat(getDato(currElement.metadata['Costo_Costo']));
		                     	}
		                     	else if(copertura.match("^2###")){
		                     		totCountExtraRowB++;
		                     		totCostiRowB+=parseFloat(getDato(currElement.metadata['Costo_Costo']));
		                     	}
		                     	else if(copertura.match("^3###")){
		                     		totCountExtraRowC++;
		                     		totCostiRowC+=parseFloat(getDato(currElement.metadata['Costo_Costo']));
		                     	}
		                     	else if(copertura.match("^4###")){
		                     		totCountExtraRowD++;
		                     		totCostiRowD+=parseFloat(getDato(currElement.metadata['Costo_Costo']));
		                     	}
		                    }
	                     }
                    	
                    	if(currElement && getDato(currElement.metadata['Rimborso_Rimborsabilita'])=='2') {
	                        if(typeof(totCountSSNCols[k])==='undefined'){
                    			totCountSSNCols[k] = 0; 
                    			totCountExtraCols[k] = 0;
                    		}
	                        totCountSSNCols[k]++;
	                     }else{
	                     	if(typeof(totCountExtraCols[k])==='undefined'){
	                     		totCountSSNCols[k] = 0; 
                    			totCountExtraCols[k] = 0;
                    		}
	                     	totCountExtraCols[k]++;
                    ...
                    if(!isNaN(parseFloat(value))){
                    	totCols[k] += parseFloat(value);
                    	totPrezzoCols[k] += parseFloat(currentPrezzoCell);
                    }
                    if(totCostiCols[k] === null || totCostiCols[k] === undefined)
                        totCostiCols[k] = 0;
                    var costo=parseFloat(getDato(currElement.metadata['Costo_Costo']));
                    	if(!isNaN(costo)){
                    		totCostiCols[k] += parseFloat(getDato(currElement.metadata['Costo_Costo']));
                    	}
                }else if (currElement){
                	console.log("count");
                	totCount++;
                	if(currElement && getDato(currElement.metadata['Rimborso_Rimborsabilita'])=='2') {
                        totCountSSNRow++;
                     	...
                     	console.log("DA QUI NON DOVREI MAI PASSARE, VERO? ASK GEORGE");
                	 	//totCountExtraRow++;
                     }
                }
            }
        ...
        totCountSSNRows[i] = totCountSSNRow;
        totCountExtraRows[i] = totCountExtraRow;
        totCountExtraRowsA[i] = totCountExtraRowA;
        totCountExtraRowsB[i] = totCountExtraRowB;
        totCountExtraRowsC[i] = totCountExtraRowC;
        totCountExtraRowsD[i] = totCountExtraRowD;
        // vmazzeo 02.05.2017 HCRPMS-461 - inizio
        totCostiRowsA[i] = totCostiRowA;
        totCostiRowsB[i] = totCostiRowB;
        totCostiRowsC[i] = totCostiRowC;
        totCostiRowsD[i] = totCostiRowD;
        // vmazzeo 02.05.2017 HCRPMS-461 - fine
    }
    if(!lastRowFound){
    ...
    $.each(totRows,function(idx,tot){
		if(idx>0 && (idx<lastRow || !lastRow)){
			try{ 
				var myPrestazioneGuid=$('#costi').handsontable('getDataAtCell',idx,0);
	            var myPrestazione=$.axmr.guid(myPrestazioneGuid);
	            if(budgetCTC) {
	            	myPrestazione.metadata['InfoBudgetPDF_TotPrezzoExtraRoutine']=totPrezzoRows[idx];
	            }else{
		            myPrestazione.metadata['InfoBudgetPDF_TotTpExtraRoutine']=totRows[idx];
		            myPrestazione.metadata['InfoBudgetPDF_TotCostoExtraRoutine']=totCostiRows[idx];
		            // vmazzeo 02.05.2017 HCRPMS-461 - inizio
		            myPrestazione.metadata['InfoBudgetPDF_TotCostoExtraRoutineA']=totCostiRowsA[idx];
		            myPrestazione.metadata['InfoBudgetPDF_TotCostoExtraRoutineB']=totCostiRowsB[idx];
		            myPrestazione.metadata['InfoBudgetPDF_TotCostoExtraRoutineC']=totCostiRowsC[idx];
		            myPrestazione.metadata['InfoBudgetPDF_TotCostoExtraRoutineD']=totCostiRowsD[idx];
		            // vmazzeo 02.05.2017 HCRPMS-461 - fine
		            myPrestazione.metadata['InfoBudgetPDF_NumTotali']=totCounts[idx];
		            myPrestazione.metadata['InfoBudgetPDF_NumSSNRoutine']=totCountSSNRows[idx];
		            myPrestazione.metadata['InfoBudgetPDF_NumExtraRoutine']=totCountExtraRows[idx];
		            myPrestazione.metadata['InfoBudgetPDF_NumExtraRoutineA']=totCountExtraRowsA[idx];
		            myPrestazione.metadata['InfoBudgetPDF_NumExtraRoutineB']=totCountExtraRowsB[idx];
		            myPrestazione.metadata['InfoBudgetPDF_NumExtraRoutineC']=totCountExtraRowsC[idx];
		            myPrestazione.metadata['InfoBudgetPDF_NumExtraRoutineD']=totCountExtraRowsD[idx];
	            }
	            $.axmr.setUpdated(myPrestazione,false);
			}catch(ex){
				console.log(ex);
			}
		}
	...
	$.each(totCols,function(idx,tot){
		if(idx>0 && (idx<lastCol || !lastCol)){
			try{ 
				var myTimePointGuid=$('#costi').handsontable('getDataAtCell',0,idx);
	            var myTimePoint=$.axmr.guid(myTimePointGuid);
	            if(budgetCTC) {
	            	myTimePoint.metadata['InfoBudgetPDF_TotPrezzoExtraRoutine']=totPrezzoCols[idx];
	            }else{
		            myTimePoint.metadata['InfoBudgetPDF_TotTpExtraRoutine']=totCols[idx];
		            myTimePoint.metadata['InfoBudgetPDF_TotCostoExtraRoutine']=totCostiCols[idx];
		            myTimePoint.metadata['InfoBudgetPDF_NumTotali']=totCountsCols[idx];
		            myTimePoint.metadata['InfoBudgetPDF_NumSSNRoutine']=totCountSSNCols[idx];
		            myTimePoint.metadata['InfoBudgetPDF_NumExtraRoutine']=totCountExtraCols[idx];
	            }
	            $.axmr.setUpdated(myTimePoint,false);
			}catch(ex){
				console.log(ex);
			}
		}
	        ...
	        $.each(docObj.elements.prezzi,function(i,prezzo){
		    	...
		    	switch(type){
		    		...
		    		case 'PrestazioneXStudio':
						var tpVal=getDato(prestazione.metadata['Costo_TransferPrice']);
						tpVal=parseFloat(tpVal);
						var quantita=parseFloat(getDato(prestazione.metadata['Costo_Quantita']));
		    			if(!isNaN(quantita)){
		    				tpVal=parseFloat(tpVal)*quantita;
		    				prezzoVal=parseFloat(prezzoVal)*quantita;
		    			}else{
		    				tpVal=0;
		    				prezzoVal=0;
		    			}
...
function addCosto(descrizione,tipologia,transfer,categoria,idx,prezzo,update){
	...
	var quantita=0;
	var myElement;
	if((categoria!==null && categoria!==undefined) || tipologia==3 || tipologia==4){
		categoria+='';
    ...
    if(tipologia==1){
        if(idx==undefined){
            ...
            myElement=docObj.elements.pxp[idx]=formToElement('dialog-form',docObj.elements.pxp[idx],folderPxp);
        }else if(update){
        	myElement=docObj.elements.pxp[idx]=formToElement('dialog-form',docObj.elements.pxp[idx],folderPxp);
        }else if(prezzo!==undefined){
        	myElement=getDato(docObj.elements.prezzi[idx].metadata['PrezzoFinale_Prestazione']);
        }else{
        ...
        if(idx==undefined){
            ...
            myElement=docObj.elements.pxs[idx]=formToElement('dialog-form',docObj.elements.pxs[idx],folderPxs);
        }else if(update){
        	myElement=docObj.elements.pxs[idx]=formToElement('dialog-form',docObj.elements.pxs[idx],folderPxs);
        }else if(prezzo!==undefined){
        	myElement=getDato(docObj.elements.prezzi[idx].metadata['PrezzoFinale_Prestazione']);
        ...
        if(idx==undefined){
            ...
            myElement=docObj.elements.pxpCTC[idx]=formToElement('dialog-form-cost-2',docObj.elements.pxpCTC[idx],folderPxpCTC);
        }else if(update){
        	myElement=docObj.elements.pxpCTC[idx]=formToElement('dialog-form-cost-2',docObj.elements.pxpCTC[idx],folderPxpCTC);
        }else{
        	myElement=docObj.elements.pxpCTC[idx];
        }
        ...
        var remove="deletePxpCTC("+idx+");";
         ...
         if(idx==undefined){
             ...
             myElement=docObj.elements.pxsCTC[idx]=formToElement('dialog-form-cost-2',docObj.elements.pxsCTC[idx],folderPxsCTC);
         }
         else if(update){
        	 myElement=docObj.elements.pxsCTC[idx]=formToElement('dialog-form-cost-2',docObj.elements.pxsCTC[idx],folderPxsCTC);
        	}else{
            	myElement=docObj.elements.pxsCTC[idx];
            }
       ...
       if(idx==undefined){
            ...
            myElement=docObj.elements.passthroughCTC[idx]=formToElement('dialog-form-cost-2',docObj.elements.passthroughCTC[idx],folderPassthroughCTC);
        }
        else if(update){
        	myElement=docObj.elements.passthroughCTC[idx]=formToElement('dialog-form-cost-2',docObj.elements.passthroughCTC[idx],folderPassthroughCTC);
        }else{
        	myElement=docObj.elements.passthroughCTC[idx];
        }
        var remove="deletePassthroughCTC("+idx+");";
    }
    quantita=parseFloat(getDato(myElement.metadata['Costo_Quantita']));
    ...
    if(!isClosed && !((tipologia==1 || tipologia==2) && prezzo!==undefined)){
    	...
    	delCol="<td	 style='text-align:center;' ><a href='#' onclick=\" formPrezzo('"+tipologia+"','"+idx+"');return false;\"><img src='/img/edit.png' width='30px'  alt='Modifica' /></a></td><td>&nbsp;</td>";
    ...
    if(tipologia!=5){
    	if(quantita && !isNaN(quantita) && quantita>0)transfer=transfer*quantita;
    	else transfer='0.00';
    }
    if(!transfer)transfer='0.00';
    if(!update){
    ...
    if(tipologia==4 || tipologia==3){
	    ...
	    $(id+" tbody tr td:nth-child(2)").each(function(){
	        console.log($(this));
	        var currValue=parseFloat($(this).html().unformatMoney());
	        if(!isNaN(currValue))tot+=currValue;
	    });
    }
    //tot=tot.toFixed(2);
    if(tipologia==5){
...
function loadCosti(){
    $.each(docObj.elements.costiAggiuntivi,function(idx,element){       
    	addCostoToTable('load',idx); 
    });
...
function addCostoToTable(form,idx){
    ...
    if(!costo){costo='0.00';}
		...
		if(!update){
			...
			$(idTrSearch).find('.descrizione-controllo').html(descrizioneControllo);
				...
				click: function() {
					...
					if(tipologia.val()==''){
						...
						return false;
		                	...
		                	bootbox.alert('Attenzione scegliere un valore per il campo "Competenza"');
		                	return false;
		            }
					else if(!$(this).find('[name=Costo_TransferPrice]').val()){
	                	bootbox.alert('Attenzione scegliere un valore per il campo "Transfer price"');
	                	return false;
					}
						...
						//(descrizione,tipologia,transfer,categoria,idx,prezzo,update)
...
function applicaSSN(){
	$.each(docObj.elements.tpxp,function(i,myElement){
		if($.isPlainObject(myElement)){
			var myPrestazione=getDato(myElement.metadata['tp-p_Prestazione']);
			if($.isPlainObject(myPrestazione)){
		    	var valoreSSN;
				if(getDato(myPrestazione.metadata['Tariffario_SSN'])!==undefined){
		    		valoreSSN=getDato(myPrestazione.metadata['Tariffario_SSN']);
		    		valoreSSN=valoreSSN.replace(/,/,'.');
		    	}
		    	var valoreTP=getDato(myElement.metadata['Costo_TransferPrice']);
		    	if(!valoreTP){
		    		myElement.metadata['Costo_Costo']=myElement.metadata['Costo_TransferPrice']=valoreSSN;
		    		$.axmr.setUpdated(myElement);
		    		$.axmr.guid(myElement,getDato(myElement.metadata['tp-p_Checked']),getDato(myElement.metadata['Costo_TransferPrice']));
		    	}
