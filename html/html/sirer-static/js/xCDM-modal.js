function xCDM_Modal(baseUrl, typeId, initFunction, compilationChecks, postSaveCallBack, loadingMessage, savingMessage,readOnly){
    ...
    this.formBySpec=function(data, parentId, elementId) {
        ...
        for (var i = 0; i < data.fieldList.length; i++) {
                        ...
                        if(optionKey=="-9999"){
                            fieldInput.change(function(){
                                if($(this).val().indexOf('-9999###')==0){
                                    $("div[data-field-ref='"+$(this).attr('name')+"Altro']").show();
                                }
                                else{
                                    $("div[data-field-ref='"+$(this).attr('name')+"Altro']").hide();
                                    $("input[name='"+$(this).attr('name')+"Altro']").val("");
                                }
                            });
                        }
                    }
            ...
            if (field.type == 'DATE') {
                ...
                fieldInputText.attr('name', field.templateName + "_" + field.fieldName).datetimepicker(
                    {
                        dateFormat: 'dd/mm/yy',
                        timeFormat:'HH:mm',
                        hourText: 'Ore',
                        ...
                        dayNames: ['Doomenica','Lunedì','Martedì','Mercoledì','Giovedì','Venerdì','Sabato'],
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.result == 'OK') {
                                ...
                                if(data.result=='ERROR' && data.errorMessage=='NOTPERMITTED'){
                                    bootbox.hideAll();
                                    bootbox.alert('Non è possibile modificare l\'elemento');
                                }
                                else {
                                    bootbox.hideAll();
    ...
    this.formByParentId=function(parentId){
        ...
        $.getJSON(baseUrl+'/app/rest/documents/'+this.typeId+'/createFormSpec', function(data){
            ...
            container.find('form').find('select').trigger("change");
    ...
    this.setFieldValue=function(fieldName, value){
        ...
        if (fieldInput.attr('type')=='radio' || fieldInput.attr('type')=='checkbox'){
            ...
            if(fieldInput.parent().hasClass('date')){
                if(fieldInput.attr('data-date-format')=='dd/mm/yyyy hh:mm'){
                    value[0]=formatDate(value[0],true);
                }
                else{
