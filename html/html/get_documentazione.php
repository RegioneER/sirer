...
from doc_obj_file f, doc_obj d, DOC_ALL_MASTER_V m 
where d.id = m.allegatocentro_id 
and d.file_id=f.id
and m.centro_id=:CENTRO_ID"; 
$bind = array();
$bind['ID']=$_POST['id'];
$bind['CENTRO_ID']=$_POST['centro_id'];
...
while ($sql->get_row()){
	...
	//echo "fwnfklnwfkw";
	echo "<span class=\"col-sm-9 x-checkbox-input x-field-ParereCe_documentazione\">
			<div class=\"checkbox\">
                <label>
                    <input autocomplete=\"off\" class=\"ace\" id=\"ParereCe_documentazione_{$sql->row['CODE']}\" name=\"ParereCe_documentazione\" value=\"{$sql->row['CODE']}###{$sql->row['DECODE']}\" title=\"{$sql->row['DECODE']}\" type=\"checkbox\" {$checked}><span class=\"lbl\">{$sql->row['DECODE']}</span>
					</input>
                </label>
            </div>
		</span>
