...
include_once "libs/db.inc";
	...
	$conn2=new dbconn("AIFA_TRASP","RSP0509#","generici_prod");
	$table="farmaci.fatc";
...
if (!isset($ATC)){
	...
	$sql->exec();
	$header_tb="<table border=1 cellpadding=0 cellspacing=0><tr><th>ATC</th><th>Descrizione</th></tr>";
	while ($sql->get_row()){
		$result.="<tr><td>{$sql->row['ATC']}</td><td><a href='sfoglia_atc.php?CODE=$CODE&DECODE=$DECODE&LIVELLO=$LIVELLO&GROUP=$GROUP&ATC={$sql->row['ATC']}'>{$sql->row['DATC']}</a></td></tr>";
	}
	$footer_tb="</table>";
