...
foreach ( $_GET as $key => $val ) {
	if (preg_match ( "!^livello_!", $key )) {
		...
		$menu_nav .= "<li><a href=\"{$script_name}?{$param}{$nav_param}&pattern={$_GET['pattern_'.$livello_nav]}&livello=$livello_nav\">$val</a></li>";
	}
}
$nav_bar="<li><a href=\"{$script_name}?{$param}\">Home</a></li>" . $menu_nav;
...
$tabella_risultati = "
	...
	<tr>
		<td class=int>&nbsp;</td>
		<td class=int>Codice ATC</td>
		<td class=int>Descrizione</td>
		<td class=int>Seleziona Specialit&agrave;</td>
	</tr>
	$tb
	</table>";
