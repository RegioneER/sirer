...
function jContr(c1,spec) {
	...
	for (j=0;j<trip.length;j++) {
		...
		if (err)
		{
			...
			if (err!=13&&err!=3&&err!=36&&err!=38) {
				er2=err;
				if (err==8&&_g['L']==1) {er2=2}
				jst=el[N+z[er2]].type;
				if (jst!='hidden'){
					N=N+z[er2]
				}
			...
			/*var controllo="fmwebContr["+tau+","+spec+"]";
			var campo="#"+N;
			controlli[campo]+=controllo;
			$(document.forms[0]).validationEngine({addedRules:{campo:controlli[campo]}});
			stop=true;lastErr=err;retu*/
			jMes_er(err,tau);
			stop=true;
			lastErr=err;
			return err;
		}else{
			$.validationEngine.closePrompt('#'+N);
			$.validationEngine.closePrompt('#'+N+'D');
			$.validationEngine.closePrompt('#'+N+'Y');
		}
		
	}
	/*for(var currControllo in controlli){
		$(document.forms[0]).validationEngine({addedRules:{currControllo:controlli[currControllo]}});
	}*/
	//stop=true;lastErr=err;return err;
	if(stop) return lastErr;
	return 0;
}


function jContr1(tau,spec) {
	...
	el=f.elements;
		...
		err=tip[tau.charAt(0)](tau);
		if (err) {jMes_er(err,tau);stop=true;lastErr=err;return decode_er(err,tau);}
...
function mes_er(er,tau,jquery) {
	...
	var errorTxt=M[_g['L']][er]+dec+c_p(tn);
	errorTxt=errorTxt.replace(/^\s*:+/,"*");
	if(!jquery)alert(M[_g['L']][50]+des+M[_g['L']][er]+dec+c_p(tn));
	else{
		$.validationEngine.buildPrompt('#'+N,errorTxt,'error');
	}
	...
	if (er!=13&&er!=3&&er!=36&&er!=38) {
		...
		jst=el[N+z[er2]].type;
		if (jst!='hidden'){
		if(jquery)$.validationEngine.buildPrompt('#'+N+z[er2],errorTxt,'error');
		el[N+z[er2]].focus()}
	}
	else {
		if (er==3||er==36) {
			...
			jst=el[N][0].type;
			if (jst!='hidden'){
			if(jquery)$.validationEngine.buildPrompt(el[N][0],errorTxt,'error');
...
function decode_er(er,tau) {
	...
	return M[_g['L']][50]+des+M[_g['L']][er]+dec+c_p(tn);
}
