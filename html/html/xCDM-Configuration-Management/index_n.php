...
if (file_exists("../serviceSpec.inc")){
    include_once "../serviceSpec.inc";
}else {
    $prefix="DOC";
}
...
function getObjectTypes($source){
    ...
    $sql="select t.id, t.type_name, t.can_be_root, t.is_self_recursive, t.check_out_enabled, t.is_container, t.searchable, t.sortable, 
        ...
        from {$prefix}_type t
        left join {$prefix}_md_field f on f.id=t.title_field_id
        left join {$prefix}_md_template tpl on tpl.id=f.template_id
        where t.deleted=0 
        order by type_name
        ";
    $q->exec($sql);
    ...
    while ($q->get_row()){
        $thisObj=$q->row;
        $sql2="select t.type_name from {$prefix}_type_{$prefix}_type dt, {$prefix}_type t where t.id=dt.allowedchilds_id and dt.doc_type_id={$q->row['ID']} order by t.type_name";
        ...
        $sql2="select t.name, dt.enabled from {$prefix}_type_md_template dt, {$prefix}_md_template t where dt.type_id={$q->row['ID']} and t.id=dt.template_id order by t.name";
        $q2=createQueryObj($source);
        $q2->exec($sql2);
        $thisObj['templates']=[];
        while ($q2->get_row()){
...
function loadTemplates($source){
    ...
    $sql="select * from (select 
        ...
        from {$prefix}_md_template t, {$prefix}_md_field f 
        where 
...
function loadAclsType($source){
    ...
    $sql="select t.type_name, da.template_ftl, da.positional_ace, dac.authority, dac.container 
        from {$prefix}_acl da, {$prefix}_acl_container dac, {$prefix}_type t 
...
function loadWFsType($source){
    ...
    $sql="select t.type_name, w.process_key, w.start_on_create, w.start_on_delete, w.start_on_update 
        from {$prefix}_type_wf w, {$prefix}_type t
...
function loadAclsTplType($source){
    global $prefix;
    $q=createQueryObj($source);
    $sql="select t.type_name, tpl.name, da.positional_ace, dac.authority, dac.container from {$prefix}_tpl_acl da, {$prefix}_tpl_acl_container dac, {$prefix}_type_md_template tt, {$prefix}_md_template tpl, {$prefix}_type t
        ...
        if(isset($array2[$key])){
            if(is_array($val) && $array2[$key]){
                ...
                if ($array2[$key]!=$array1[$key]){
                    if ($key=='AVAILABLE_VALUES'){
                        if (isJson($array1[$key])){
                            $obj1=json_decode($array1[$key], true);
                            $obj2=json_decode($array2[$key], true);
                            $diff=check_diff_multi($obj1, $obj2);
                            if (count($diff)!=0){
                                $result[$key]=$val;
                            }
                        }
