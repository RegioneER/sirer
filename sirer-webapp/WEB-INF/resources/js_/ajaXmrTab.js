...
function ajaXmrTab(options){
    ...
    this.init = function(){
            ...
            $.ajax({
                ...
                success: function(obj){
                    if (obj.result=="OK") {
                        ...
                        $( "#"+currObj.elementName+"-dialog" ).dialog( "close" );
        ...
        $('#add-'+this.elementName).click(function(){
            ...
            $( "#"+currObj.elementName+"-dialog" ).dialog( "open" );
        });
    }
    ...
    this.appendDeleteIcon=function(id, container){
        ...
        if (currObj.deleteUrl!=null)
            $(container).append('<a href="#" class="ui-icon ui-icon-trash '+currObj.elementName+'-delete" style="display: inline-block" title="elimina">'+id+'</a>');
        ...
        $('.'+currObj.elementName+'-delete').click(function(){
            if (confirm("Sei sicuro")){
                ...
                $.ajax({
                    type: "GET",
                    url: currObj.deleteUrl+'/'+$(this).html(),
                    success: function(obj){
                        if (obj.result=="OK") {
    ...
    this.appendEditIcon=function(id, container){
        var currObj=this;
        if (currObj.getSingleElementUrl!=null) $(container).append('<a href="#" class="ui-icon ui-icon-pencil '+currObj.elementName+'-edit" style="display: inline-block" title="modifica">'+id+'</a>');
        $('.'+currObj.elementName+'-edit').click(function(){
            if (currObj.editPage!=null && currObj.editPage!="") {
                window.location.href=currObj.editPage+'/'+$(this).html();
                return;
            }
            $.getJSON(currObj.getSingleElementUrl+'/'+$(this).html(), function(data) {
    ...
    this.refreshList=function(){
        ...
        $.getJSON(currObj.getAllUrl, function(data) {
            ...
            for (i=0;i<data.length;i++){
                ...
                if (currObj.getSingleElementUrl!=null) list+='<a href="#" class="ui-icon ui-icon-pencil '+currObj.elementName+'-edit" style="display: inline-block" title="modifica">'+data[i].id+'</a> ';
                if (currObj.deleteUrl!=null)list+='<a href="#" class="ui-icon ui-icon-trash '+currObj.elementName+'-delete" style="display: inline-block" title="elimina">'+data[i].id+'</a>';
                if ((currObj.getSingleElementUrl!=null || currObj.deleteUrl!=null) && currObj.listType=='tr') list+="</td>";
                if (currObj.listType=='li') list+="</li>";
                if (currObj.listType=='tr') list+="</tr>";
